...
class PhysxRigidBodyComponentStruct(PhysxRigidBaseComponentStruct[T], Generic[T]):
    ...
    def get_net_contact_impulses(self):
        ...
        if self.scene.gpu_sim_enabled:
            ...
            body_contacts = sapien_utils.get_cpu_actor_contacts(self.px.get_contacts(), self._bodies[0].entity)
            net_force = common.to_tensor(sapien_utils.compute_total_impulse(body_contacts))
    ...
    def cmass_local_pose(self) -> Pose:
        raw_poses = np.stack([np.concatenate([x.cmass_local_pose.p, x.cmass_local_pose.q]) for x in self._bodies])
...
class PhysxRigidDynamicComponentStruct(PhysxRigidBodyComponentStruct[T], Generic[T]):
    ...
    def angular_velocity(self, arg1: Array):
        if self.scene.gpu_sim_enabled:
            ...
            self._body_data[self._body_data_index[self.scene._reset_mask[self._scene_idxs]], 10:13] = arg1
        else:
    ...
    def gpu_pose_index(self):
        if self.scene.gpu_sim_enabled:
            ...
            raise AttributeError("GPU pose index is not supported when gpu is not enabled")
    ...
    def linear_velocity(self, arg1: Array):
        if self.scene.gpu_sim_enabled:
            arg1 = common.to_tensor(arg1, device=self.device)
            self._body_data[self._body_data_index[self.scene._reset_mask[self._scene_idxs]], 7:10] = arg1
        else:
            arg1 = common.to_numpy(arg1)
            if len(arg1.shape) == 2:
                arg1 = arg1[0]
    ...
    def locked_motion_axes(self) -> Array:
        ...
        :type: list[bool]
        """
        return torch.tensor([body.locked_motion_axes for body in self._bodies], device=self.device)
