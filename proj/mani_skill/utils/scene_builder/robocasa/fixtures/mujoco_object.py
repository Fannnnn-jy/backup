...
class MujocoObject:
    ...
    def __init__(
        ...
        self.loader.visual_groups = [1]  # for robocasa, 1 is visualized, 0 is collisions
        if xml is not None:
            orig_xml = xml
            ...
            if not xml.exists():
                ...
                parsed = self.loader.parse(xml, package_dir=xml / "../")
            assert len(parsed["articulation_builders"]) + len(parsed["actor_builders"]) == 1, (
        ...
        if hasattr(self, "articulation_builder"):
            for link in self.articulation_builder.link_builders:
                for visual in link.visual_records:
                    visual.pose = sapien.Pose(p=np.multiply(visual.pose.p, scale), q=visual.pose.q)
    ...
    def get_bbox_points(self, trans=None, rot=None):
        ...
        horizontal_radius_site = self.loader.xml.find(f".//site[@name='{self.naming_prefix}horizontal_radius_site']")
    ...
    def bottom_offset(self):
        bottom_site = self.loader.xml.find(f".//site[@name='{self.naming_prefix}bottom_site']")
        return string_to_array(bottom_site.get("pos")) * self._scale

    @property
    def top_offset(self):
        top_site = self.loader.xml.find(f".//site[@name='{self.naming_prefix}top_site']")
