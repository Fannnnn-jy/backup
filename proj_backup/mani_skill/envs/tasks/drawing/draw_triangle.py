...
class DrawTriangleEnv(BaseEnv):
    ...
    **Success Conditions:**
    ...
    _sample_video_link = (
        "https://github.com/haosulab/ManiSkill/raw/main/figures/environment_demos/DrawTriangle-v1_rt.mp4"
    )
    ...
    def _load_scene(self, options: dict):
        ...
        def create_goal_triangle(name="tri", base_color=None):
            ...
            first_block_pose = sapien.Pose(list(c1), euler2quat(0, 0, theta - (np.pi / 2)))
            ...
            second_block_pose = sapien.Pose(list(c2), euler2quat(0, 0, theta - (5 * np.pi / 6)))
            ...
            third_block_pose = sapien.Pose(list(c3), euler2quat(0, 0, theta - (np.pi / 6)))
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            if hasattr(self, "vertices"):
                self.vertices[env_idx] = torch.from_numpy(np.tile(self.original_verts, (b, 1, 1))).to(
                    self.device
                )  # b, 3, 3
            else:
                self.vertices = torch.from_numpy(np.tile(self.original_verts, (b, 1, 1))).to(self.device)

            self.vertices[env_idx] = (mats.double() @ self.vertices[env_idx].transpose(-1, -2).double()).transpose(
                -1, -2
            )  # apply rotation matrix
            self.vertices[env_idx] += target_pos.unsqueeze(1)

            self.triangles = self.generate_triangle_with_points(50, self.vertices[:, :, :-1])
            ...
            for dot in self.dots:
                ...
                dot.set_pose(sapien.Pose(p=[0, 0, -self.DOT_THICKNESS], q=euler2quat(0, np.pi / 2, 0)))
    ...
    def success_check(self):
        if self.draw_step > 0:
            current_dot = self.dots[self.draw_step - 1].pose.p.reshape(self.num_envs, 1, 3)  # b,3
            ...
            self.ref_dist = torch.logical_or(self.ref_dist, (1 - z_mask.int()) * dist.reshape((self.num_envs, 153)))

            # if current drawn point is close to a reference point. -1 if the drawn point hasn't actually been drawn yet
            self.dots_dist[:, self.draw_step - 1] = torch.where(z_mask, -1, torch.any(dist, dim=-1)).reshape(
                self.num_envs,
            )
