...
FIXTURES_INTERIOR = dict(sink=Sink, stovetop=Stovetop, accessory=Accessory, wall_accessory=WallAccessory)
...
def check_syntax(fixture):
    ...
    if "align_to" in fixture or "side" in fixture or "alignment" in fixture:
        if not ("align_to" in fixture and "side" in fixture):
            raise ValueError('Both or neither of "align_to" and "side" need to be specified.')
        if "pos" in fixture:
...
class RoboCasaSceneBuilder(SceneBuilder):
    ...
    def build(self, build_config_idxs: list[int] | None = None):
        ...
        for scene_idx, build_config_idx in enumerate(build_config_idxs):
            ...
            (
                ...
                robot_base_pos,
                robot_base_ori,
            ) = self._generate_initial_placements(fixtures, fixture_cfgs, rng=self.env._batched_episode_rng[scene_idx])
            ...
            if self.env.agent is not None:
                self.robot_poses.raw_pose[scene_idx][:3] = torch.from_numpy(robot_base_pos).to(self.robot_poses.device)
                self.robot_poses.raw_pose[scene_idx][3:] = torch.from_numpy(euler2quat(*robot_base_ori)).to(
                    self.robot_poses.device
                )
                ...
                if built is not None:
                    ...
                    if built.is_articulation and built.articulation.fixed_root_link.all():
                        ...
                        for link in built.articulation.links:
                            ...
                            link.set_collision_group(group=2, value=0)  # clear all default ignored collisions
                            if link.joint.type[0] == "fixed":
                                link.set_collision_group_bit(group=2, bit_idx=actor_bit, bit=1)
                            link.set_collision_group_bit(group=2, bit_idx=collision_bit, bit=1)
                        ...
                        if built.actor.px_body_type == "static":
                            ...
                            if isinstance(v, Floor):
                                for bit_idx in range(21, 32):
                                    built.actor.set_collision_group_bit(group=2, bit_idx=bit_idx, bit=1)
                                ...
                                built.actor.set_collision_group_bit(group=2, bit_idx=actor_bit, bit=1)
            # self.actors = actors
    ...
    def _generate_initial_placements(self, fixtures, fixture_cfgs, rng: np.random.RandomState):
        ...
        if self.env.agent is not None:
            robot_base_pos, robot_base_ori = self.compute_robot_base_placement_pose(fixtures, ref_fixture)
        else:
            robot_base_pos = None
            robot_base_ori = None
        ...
        with torch.device(self.env.device):
            if self.env.agent is not None:
                if self.env.robot_uids == "fetch":
                    ...
                    self.env.agent.robot.set_qpos(self.env.agent.keyframes["standing"].qpos)
                    xyz = self.env.agent.robot.pose.p
                    xyz[:, 2] += self.env.agent.keyframes["standing"].pose.p[2]
                    self.env.agent.robot.set_pose(Pose.create_from_pq(p=xyz, q=self.env.agent.robot.pose.q))
    ...
    def get_fixture(self, fixtures, id, ref=None, size=(0.2, 0.2)):
        ...
        if ref is None:
            ...
            if isinstance(id, FixtureType) or isinstance(id, int):
                matches = [name for (name, fxtr) in fixtures.items() if fixture_is_type(fxtr, id)]
            else:
                matches = [name for name in fixtures.keys() if id in name]
            if id == FixtureType.COUNTER or id == FixtureType.COUNTER_NON_CORNER:
                matches = [name for name in matches if self._is_fxtr_valid(fixtures[name], size)]
    ...
    def _get_placement_initializer(
        ...
        for obj_i, cfg in enumerate(cfg_list):
            ...
            if fixture_id is not None:
                ...
                reset_region = fixture.sample_reset_region(env=self, fixtures=fixtures, **sample_region_kwargs)
                ...
                if inner_xpos == "ref":
                    ...
                    if x_halfsize == 0.0:
                        ...
                        ref_fixture = self.get_fixture(fixtures, placement["sample_region_kwargs"]["ref"])
                ...
                x_range = (
                    np.array([-inner_size[0] / 2, inner_size[0] / 2]) + reset_region["offset"][0] + intra_offset[0]
                )
                y_range = (
                    np.array([-inner_size[1] / 2, inner_size[1] / 2]) + reset_region["offset"][1] + intra_offset[1]
                )
