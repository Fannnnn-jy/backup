...
class GleamDataset:
    ...
    def _load(self) -> None:
        ...
        range_gt = torch.load(gt_dir / f"{dataset}_{self.grid_size}_range_gt.pt", map_location="cpu")
        voxel_size = torch.load(gt_dir / f"{dataset}_{self.grid_size}_voxel_size_gt.pt", map_location="cpu")
    ...
    def _load_init_positions(self, gt_dir: Path, range_gt: "torch.Tensor") -> Optional[List[np.ndarray]]:
        dataset = self.dataset_name
