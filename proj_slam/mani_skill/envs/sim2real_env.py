...
class Sim2RealEnv(gym.Env):
    ...
    def __init__(
        ...
        sim_env: gym.Env,
        agent: BaseRealAgent,
        real_reset_function: Callable[["Sim2RealEnv", int | None, dict | None], None] | None = None,
        sensor_data_preprocessing_function: Callable[[dict], dict] | None = None,
        ...
        self.num_envs = 1
        assert self.sim_env.unwrapped.backend.sim_backend == "physx_cpu", (
    ...
    def _step_action(self, action):
        ...
        if self.last_control_time is None:
            ...
            if dt < self.control_dt:
                ...
                logger.warning(f"Control dt {self.control_dt} was not reached, actual dt was {dt}")
        self.last_control_time = time.perf_counter()
    ...
    def _get_obs_with_sensor_data(self, info: dict, apply_texture_transforms: bool = True) -> dict:
        ...
        return self.base_sim_env.__class__._get_obs_with_sensor_data(self, info, apply_texture_transforms)
    ...
    def compute_sparse_reward(self, obs: Any, action: torch.Tensor, info: dict):
        ...
        return self.base_sim_env.__class__.compute_sparse_reward(self, obs, action, info)
        ...
        def check_observation_match(sim_obs, real_obs, path=[]):
            ...
            if isinstance(sim_obs, dict):
                for key in sim_obs.keys():
                    ...
                    check_observation_match(sim_obs[key], real_obs[key], path=path + [key])
            else:
    ...
    def preprocess_sensor_data(self, sensor_data: dict, sensor_names: list[str] | None = None):
        import cv2
            ...
            for key in ["rgb", "depth"]:
                if key in real_sensor_data:
                    ...
                    if xy_res[0] == xy_res[1]:
                        ...
                        img = img[:, cutoff:-cutoff, :]
                    real_sensor_data[key] = common.to_tensor(cv2.resize(img, (target_w, target_h))).unsqueeze(0)
