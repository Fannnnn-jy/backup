...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Active mapping viewer for GLB scenes.")
    ...
    parser.add_argument("--topdown", action="store_true", help="Render an additional top-down camera view.")
    parser.add_argument(
        "--init-pos",
        type=_parse_vec3,
        default=(0, 0, 0),
        help="Initial position x,y[,z] (active_mapping mode). If z omitted, use default height.",
    )
    ...
    parser.add_argument(
        "--am-action-mode",
        default="",
        help="Active mapping action mode (xy/xyz/xy_cos_sin). Empty = config default.",
    )
    parser.add_argument(
        "--am-camera-angles",
        default="",
    ...
    parser.add_argument("--am-single-camera", action="store_true", help="Use first camera only (active_mapping mode).")
...
def _log_trajectory_rerun(rr, positions: np.ndarray, log_every: int) -> None:
    if rr is None:
        return
    if log_every <= 0:
        log_every = 1
    if positions.shape[0] % log_every != 0:
        return
    rr.log("trajectory", rr.LineStrips3D([positions]))
    rr.log("trajectory/current", rr.Points3D(positions[-1:]))
...
def _run_active_mapping(args: argparse.Namespace) -> None:
    ...
    def _record_pose(position: np.ndarray, quat: np.ndarray) -> None:
        ...
        _log_trajectory_rerun(rr, positions, args.rerun_log_every)
    ...
    view_mode = "both" if args.topdown else "depth"
    ...
    print("Controls: WASD = move (world X/Y), K/L = +Z/-Z (xyz only), 1/2 = yaw (xy_cos_sin), Q/Esc = quit")
    ...
    while True:
        ...
        if key in (ord("w"), ord("W")):
            ...
            delta[1] += move_step
            moved = True
        elif key in (ord("k"), ord("K"), ord("l"), ord("L")) and mode != "xyz":
            ...
            continue
        elif key in (ord("k"), ord("K")) and mode == "xyz":
            delta[2] += move_step
            moved = True
        elif key in (ord("l"), ord("L")) and mode == "xyz":
            delta[2] -= move_step
            moved = True
        elif key == ord("1"):
            yaw += rot_step
