...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--rerun-mesh", action="store_true", default=False, help="Log the GLB mesh to Rerun.")
    parser.add_argument(
        "--rerun-mesh-mode",
...
def _log_voxels_rerun(
    rr,
    env,
    accumulated_pointcloud: np.ndarray,
    root: str,
) -> None:
    """Log voxels to Rerun: gray for obstacle voxels, purple for those covered by pointcloud.

    Point-cloud-to-voxel mapping uses the same convention as
    ``reconstruct_and_eval._mark_covered``:
        voxel_idx = floor((point - grid_origin) / pitch)
    where *grid_origin* is the corner of the (0,0,0) voxel, **not** its centre.
    ``_range_gt`` stores voxel *centres*, so grid_origin = range_min - 0.5 * pitch.
    """
    if rr is None:
        return
    obstacle_mask = getattr(env, "_obstacle_mask_3d", None)
    ...
    pitch = float(voxel_size[0])
    # _range_gt stores voxel centres; recover grid corner origin
    # (matches reconstruct_and_eval._mark_covered / _load_voxel_grid origin)
    origin = np.array([
        float(range_gt[1]) - 0.5 * pitch,  # x_min centre → corner
        float(range_gt[3]) - 0.5 * pitch,  # y_min centre → corner
        float(range_gt[5]) - 0.5 * pitch,  # z_min centre → corner
    ], dtype=np.float32)

    # --- Mark voxels covered by accumulated point cloud (same as _mark_covered) ---
    covered_mask = np.zeros_like(obstacle_mask, dtype=bool)
    pts = np.asarray(accumulated_pointcloud, dtype=np.float32)
    if pts.size > 0 and pts.ndim == 2 and pts.shape[1] == 3:
        idx = np.floor((pts - origin) / pitch).astype(np.int64)
        gx, gy, gz = obstacle_mask.shape
        valid = (
            (idx[:, 0] >= 0) & (idx[:, 0] < gx) &
            (idx[:, 1] >= 0) & (idx[:, 1] < gy) &
            (idx[:, 2] >= 0) & (idx[:, 2] < gz)
    ...
    # Dilate covered_mask by 1 voxel (6-connectivity) to tolerate point-cloud
    # sparsity, matching reconstruct_and_eval.py coverage_dilation logic.
    from scipy.ndimage import binary_dilation
    struct = np.zeros((3, 3, 3), dtype=bool)
    struct[1, 1, 1] = True
    struct[1, 1, 0] = struct[1, 1, 2] = True  # ±z
    struct[1, 0, 1] = struct[1, 2, 1] = True  # ±y
    struct[0, 1, 1] = struct[2, 1, 1] = True  # ±x
    covered_mask = binary_dilation(covered_mask, structure=struct)

    # Uncovered obstacle voxels (gray) and covered obstacle voxels (purple)
    covered  = obstacle_mask & covered_mask
    uncovered = obstacle_mask & ~covered_mask

    uncovered_indices = np.argwhere(uncovered)  # (N, 3) ijk
    covered_indices   = np.argwhere(covered)    # (M, 3) ijk

    def _ijk_to_world(ijk: np.ndarray) -> np.ndarray:
        """Convert voxel indices to world-frame voxel centres."""
        if ijk.shape[0] == 0:
            return np.zeros((0, 3), dtype=np.float32)
        return origin + (ijk.astype(np.float32) + 0.5) * pitch

    half = pitch * 0.25
    voxel_path = f"{root}/voxels"

    if uncovered_indices.shape[0] > 0:
        centers = _ijk_to_world(uncovered_indices)
        gray = np.full((centers.shape[0], 4), [160, 160, 160, 100], dtype=np.uint8)
        rr.log(f"{voxel_path}/obstacle", rr.Points3D(centers, colors=gray, radii=half))
    else:
        rr.log(f"{voxel_path}/obstacle", rr.Clear(recursive=False))

    if covered_indices.shape[0] > 0:
        centers = _ijk_to_world(covered_indices)
        purple = np.full((centers.shape[0], 4), [160, 32, 240, 200], dtype=np.uint8)
        rr.log(f"{voxel_path}/covered", rr.Points3D(centers, colors=purple, radii=half))
    else:
        rr.log(f"{voxel_path}/covered", rr.Clear(recursive=False))
...
def _run_active_mapping(args: argparse.Namespace) -> None:
    ...
    while True:
        ...
        if key in (ord("c"), ord("C")):
            ...
            _log_voxels_rerun(rr, env, accumulated_pointcloud, rerun_root)
            continue
