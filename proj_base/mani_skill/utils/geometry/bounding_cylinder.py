...
def _compute_smallest_circle(points: list[tuple[float, float]]) -> tuple[float, float, float] | None:
...
def _compute_circle_with_point(points: list[tuple[float, float]], p: tuple[float, float]) -> tuple[float, float, float]:
    circle = (p[0], p[1], 0.0)
...
def _compute_circle_with_two_points(
    ...
    for r in points:
        ...
        if candidate is None:
            ...
            left is None
            or _compute_cross_product(p[0], p[1], q[0], q[1], candidate[0], candidate[1])
...
def _get_circle_from_diameter(a: tuple[float, float], b: tuple[float, float]) -> tuple[float, float, float]:
    center_x = (a[0] + b[0]) / 2
    center_y = (a[1] + b[1]) / 2
    radius = max(
...
def _compute_circumcircle(
    ...
    center_x = (
        mid_x
        + ((ax * ax + ay * ay) * (by - cy) + (bx * bx + by * by) * (cy - ay) + (cx * cx + cy * cy) * (ay - by)) / det
    )
    center_y = (
        mid_y
        + ((ax * ax + ay * ay) * (cx - bx) + (bx * bx + by * by) * (ax - cx) + (cx * cx + cy * cy) * (bx - ax)) / det
    )
...
def _point_in_circle(circle: tuple[float, float, float] | None, point: tuple[float, float]) -> bool:
    if circle is None:
        return False
    return math.hypot(point[0] - circle[0], point[1] - circle[1]) <= circle[2] * (1 + 1e-14)


def _compute_cross_product(x0: float, y0: float, x1: float, y1: float, x2: float, y2: float) -> float:
