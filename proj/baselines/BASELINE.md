...
export PYTHONPATH=/home/ghr/fs/Junyi/GLEAM/isaacgym/python:$PYTHONPATH
### GLEAM 轨迹生成
...
```
python /home/ghr/fs/Junyi/proj/baselines/gleam_poses/preprocess_normalized_glb.py \
    --glb /home/ghr/fs/Junyi/data/proj/actrec_data/dst_data/replicacad/apt_0.glb \
    --scene_id replicacad/apt_0 \
    --drone_height 1.5 \
    --grid_reso 128 \
    --work_dir /home/ghr/fs/Junyi/data/proj/baseline_data/tmp \
    --overwrite \
    --input_glb_mode raw
```
...
可以指定固定起始位置（Z-up 坐标）和朝向（度数，绕 +Z 轴 CCW），用于与其他 baseline 对齐采样起点：
...
| `--init_x` | None | 固定起始 X（Z-up 坐标），不填则从 init_map 随机采样 |
| `--init_y` | None | 固定起始 Y（Z-up 坐标），不填则从 init_map 随机采样 |
| `--init_yaw_deg` | None | 固定起始朝向（度，绕 +Z CCW），不填则使用环境默认重置朝向 |
...
python baselines/render_from_poses.py \
    --pose_json /home/ghr/fs/Junyi/proj/baselines/poses/gleam-gt/replicacad__apt_0_run_000.json \
