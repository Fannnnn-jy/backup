...
class WandbOnPolicyRunner:
    ...
    def _log_render_images(self, it: int) -> None:
        ...
        if pending is not None:
            ...
            if action_val is not None:
                action_str = np.array2string(
                    np.asarray(action_val), precision=3, floatmode="fixed"
                )
        ...
        if depth_img is not None:
            payload["render/depth"] = self._wandb.Image(depth_img[..., ::-1])
        if topdown_img is not None:
            payload["render/topdown"] = self._wandb.Image(
                topdown_img[..., ::-1], caption=caption
            )
        if payload:
