...
class RenderCamera:
    ...
    def create(
        ...
        return cls(_render_cameras=render_cameras, scene=scene, name=shared_name, mount=mount)
    ...
    def get_extrinsic_matrix(self):
        if self.scene.gpu_sim_enabled:
            ...
            res = (ros2opencv @ self.get_global_pose().inv().to_transformation_matrix())[:, :3, :4]
            if self.mount is None:
                self._cached_extrinsic_matrix = res
            return res
        else:
            return common.to_tensor(self._render_cameras[0].get_extrinsic_matrix())[None, :]
    ...
    def get_local_pose(self) -> Pose:
        if self._cached_local_pose is None:
            if self.scene.gpu_sim_enabled:
                ps = np.array([
                    np.concatenate([cam.get_local_pose().p, cam.get_local_pose().q]) for cam in self._render_cameras
                ])
                self._cached_local_pose = Pose.create(common.to_tensor(ps), device=self.scene.device)
            else:
    ...
    def get_model_matrix(self):
        if self.scene.gpu_sim_enabled:
            ...
            qmat[:, -1, -1] = 1
            pmat = torch.eye(4, device=self.scene.device)[None, ...].repeat(len(qmat), 1, 1)
            pmat[:, :3, 3] = pose.p
            res = pmat @ qmat
            if self.mount is None:
    ...
    def get_picture(self, names: str | list[str]) -> list[torch.Tensor]:
        if isinstance(names, str):
            names = [names]
        if self.scene.gpu_sim_enabled and not self.scene.parallel_in_single_scene:
            if SAPIEN_RENDER_SYSTEM == "3.0":
                return [self.camera_group.get_picture_cuda(name).torch() for name in names]
            ...
            if self.scene.backend.render_backend == "sapien_cuda":
                return [self._render_cameras[0].get_picture_cuda(name).torch()[None, ...] for name in names]
            else:
                return [common.to_tensor(self._render_cameras[0].get_picture(name))[None, ...] for name in names]
