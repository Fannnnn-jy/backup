...
# call reconstruct_and_eval_wint3r.py to run WinT3R incrementally and compute metrics.
#
# Prerequisites: rendered frames must already exist (run run_render.sh first).
#
# Output: baselines/reconstructions/ and baselines/results/ with *_wint3r suffixes.
#
# Usage:
#   bash baselines/run_recon_eval_wint3r.sh
#   bash baselines/run_recon_eval_wint3r.sh                          # all methods, all scenes
#   METHODS="gleam active-gs-gt-depth" bash baselines/run_recon_eval_wint3r.sh
#   SCENES="replicacad/apt_0" bash baselines/run_recon_eval_wint3r.sh
#   OVERWRITE=true bash baselines/run_recon_eval_wint3r.sh
#   SAVE_RECON_ARTIFACTS=false bash baselines/run_recon_eval_wint3r.sh
...
echo
echo "==== run_recon_eval_wint3r done: ${n_run} run, ${n_skip} skipped, ${n_missing_render} missing render, ${n_total} total ===="
