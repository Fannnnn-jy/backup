...
class FrankaBenchmarkEnv(DirectRLEnv):
        ...
        for i in range(num_cameras):
            tiled_camera_cfg = TiledCameraCfg(
                ...
                offset=TiledCameraCfg.OffsetCfg(
                    pos=(-0.4, 0.0, 1.0), rot=(0.9689124, 0.0, 0.247404, 0.0), convention="world"
                ),
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        self.robot_dof_targets[:] = torch.clamp(
            self.actions + self._robot.data.joint_pos, self.robot_dof_lower_limits, self.robot_dof_upper_limits
        )
