...
def add_white_ceiling_open3d(
    ...
    """Add a white ceiling box above current geometry bounds."""
    bounds = _get_open3d_bounds(geometries)
    if bounds is None or not np.isfinite(bounds).all():
...
def add_white_ceiling_trimesh(
    ...
    """Add a white ceiling box above current scene bounds."""
    if scene is None or not scene.geometry:
    ...
    white_material = trimesh.visual.material.PBRMaterial(
        ...
        roughnessFactor=1.0,
        doubleSided=True,
        # Keep a small emissive term so ceiling stays visible in low-light viewers.
...
def parse_replica_cad_scene_trimesh(
    ...
    Args:
        ...
        normalize_axes: Normalize to y:[0,1], x/z:[-1,1]
    """
    print(f"Loading scene configuration from: {scene_config_path}")
...
def main(
    ...
    if vis or (save_path and not (export_textures and save_ext == ".glb")):
        ...
        geometries = parse_replica_cad_scene(
            scene_cfg,
            asset_dir,
