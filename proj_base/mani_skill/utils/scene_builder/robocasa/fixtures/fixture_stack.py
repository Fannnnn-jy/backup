...
class FixtureStack:
    ...
    def _create_stack(self):
        ...
        if self.base_height > 0:
            ...
            base_config = scene_utils.load_style_config(self.scene_style, {"type": "box"})
            ...
            base = scene_utils.initialize_fixture(self.scene, base_config, self.scene_fixtures)
        ...
        for i, level in enumerate(self.config["levels"]):
            ...
            for fixture, (x, width) in fixtures:
                ...
                fxtr_config = scene_utils.load_style_config(self.scene_style, {"type": fixture})
                ...
                fixture = scene_utils.initialize_fixture(self.scene, fxtr_config, self.scene_fixtures, rng=self.rng)
    ...
    def _check_config_syntax(config):
        ...
        if "levels" not in config or "percentages" not in config:
            raise ValueError('Both "levels" and "percentages" must be specified for fixture stack')
        if len(config["levels"]) != len(config["percentages"]):
            raise ValueError('"levels" and "percentages" must have the same length for fixture stacks')
        for level in config["levels"]:
            if type(level) == list and len(level) != 2:
                raise ValueError("There can be at most 2 fixtures per level in fixture stacks")
