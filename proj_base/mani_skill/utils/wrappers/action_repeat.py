...
class ActionRepeatWrapper(gym.Wrapper):
    ...
    def step(self, action):
        final_obs, final_rew, final_terminations, final_truncations, infos = super().step(action)
        ...
        if not_dones.any():
            for _ in range(self.repeat - 1):
                new_obs, new_rew, new_terminations, new_truncations, new_infos = super().step(action)
