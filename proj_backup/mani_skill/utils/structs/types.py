...
class GPUMemoryConfig:
    ...
    max_rigid_patch_count: int = 2**18  # 81920 is SAPIEN default but most tasks work with 2**18
    """Increase this if you get 'Patch buffer overflow detected'"""
    heap_capacity: int = 2**26
    found_lost_pairs_capacity: int = 2**25  # 262144 is SAPIEN default but most tasks work with 2**25
...
@dataclass
class SceneConfig:
    gravity: np.ndarray | list[float] = field(default_factory=lambda: np.array([0, 0, -9.81]))
