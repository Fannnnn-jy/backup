...
DEFAULT_PRED_BASELINE_POSE_METHOD = "gleam-pred-depth"
...
def main():
    additional_args = [
        ...
        {
            "name": "--custom_disable_pose_export",
            "type": bool,
            "default": False,
            "help": "Disable exporting baseline pose JSON files.",
        },
        {
            "name": "--custom_pred_depth",
            "type": bool,
            "default": False,
            "help": "Use DepthAnything-predicted depth to replace simulator depth observations.",
        },
        {
            "name": "--custom_pred_depth_model",
            "type": str,
            "default": "depth-anything/da3mono-large",
            "help": "DepthAnything model name used when custom_pred_depth is enabled.",
        },
        {
            "name": "--custom_pred_depth_cache_dir",
            "type": str,
            "default": "/home/ghr/fs/Junyi/model_weights",
            "help": "Cache directory for the DepthAnything model.",
        },
    ]
    reward_args = [
        {
    ...
    if args.custom_pred_depth and baseline_pose_method == DEFAULT_BASELINE_POSE_METHOD:
        baseline_pose_method = DEFAULT_PRED_BASELINE_POSE_METHOD
    Env_GLEAM_Custom.configure(
        ...
        baseline_pose_method=baseline_pose_method,
        enable_baseline_pose_export=not args.custom_disable_pose_export,
        pred_depth=args.custom_pred_depth,
        pred_depth_model_name=args.custom_pred_depth_model,
        pred_depth_cache_dir=args.custom_pred_depth_cache_dir,
    )
    ...
    if args.custom_disable_pose_export:
        print("[custom] baseline pose export: disabled")
    else:
        print(
            f"[custom] baseline pose export: root={args.custom_baseline_pose_root} "
            f"method={baseline_pose_method}"
        )
    print(
        f"[custom] pred_depth={args.custom_pred_depth} "
        f"model={args.custom_pred_depth_model}"
    )
