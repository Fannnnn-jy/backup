...
class Window:
    ...
    def build(self, scene_idxs: list[int]):
        ...
        self.actor_builder.initial_pose = sapien.Pose(p=np.array(self.pos), q=euler2quat(*self.euler))
        self.actor = self.actor_builder.build_static(name=self.name + f"_{scene_idxs[0]}")
        return self
    ...
    def create_window(self):
        ...
        for i, (obj_name, size) in enumerate(zip(names, sizes)):
            ...
            if "door" in obj_name:
                ...
                # )
                builder.add_box_collision(half_size=size, pose=obj_pose)
                builder.add_box_visual(half_size=size, name=obj_name, material=self.trim_mat, pose=obj_pose)
...
class FramedWindow(Window):
    ...
    def create_window(self):
        ...
        for obj_name, size in zip(names, sizes):
            ...
            builder.add_box_visual(half_size=size, name=obj_name, material=self.trim_mat)
