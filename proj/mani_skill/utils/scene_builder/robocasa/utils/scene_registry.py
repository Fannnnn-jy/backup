...
def get_layout_path(layout_id):
    ...
    if isinstance(layout_id, int):
        layout_int_to_name = dict(map(lambda item: (item.value, item.name.lower()), LayoutType))
...
def get_style_path(style_id):
    ...
    if isinstance(style_id, int):
        ...
        raise ValueError
    return ASSET_DIR / f"scene_datasets/robocasa_dataset/assets/scenes/kitchen_styles/{style_name}.yaml"
