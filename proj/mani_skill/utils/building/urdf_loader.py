...
class URDFLoader(SapienURDFLoader):
    ...
    def parse(self, urdf_file, srdf_file=None, package_dir=None) -> ParsedURDFData:
        articulation_builders, actor_builders, cameras = super().parse(urdf_file, srdf_file, package_dir)
    ...
    def load_file_as_articulation_builder(self, urdf_file, srdf_file=None, package_dir=None) -> ArticulationBuilder:
        return super().load_file_as_articulation_builder(urdf_file, srdf_file, package_dir)
    ...
    def load(
        ...
        if len(articulation_builders) > 1 or len(actor_builders) != 0:
            raise Exception("URDF contains multiple objects, call load_multiple instead")
        ...
        if len(cameras) > 0:
            ...
            for scene_idx, scene in enumerate(self.scene.sub_scenes):
                for cam in cameras:
                    ...
                    name2entity[f"scene-{scene_idx}_{cam['reference']}"].add_component(cam_component)
