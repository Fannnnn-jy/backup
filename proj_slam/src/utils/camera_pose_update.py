...
def quat_wxyz_from_yaw_pitch(yaw_rad: float, pitch_rad: float) -> np.ndarray:
    half_yaw = 0.5 * float(yaw_rad)
    half_pitch = 0.5 * float(pitch_rad)
    yaw_quat = np.array([np.cos(half_yaw), 0.0, 0.0, np.sin(half_yaw)], dtype=np.float32)
    pitch_quat = np.array([np.cos(half_pitch), 0.0, np.sin(half_pitch), 0.0], dtype=np.float32)
    return quat_mul_wxyz(yaw_quat, pitch_quat)


def project_quat_wxyz_to_yaw_pitch_only(quat: np.ndarray) -> np.ndarray:
    rot = quat_wxyz_to_matrix(quat)
    yaw = float(np.arctan2(rot[1, 0], rot[0, 0]))
    pitch = float(np.arctan2(-rot[2, 0], np.hypot(rot[0, 0], rot[1, 0])))
    return quat_wxyz_from_yaw_pitch(yaw, pitch)
