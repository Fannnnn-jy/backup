...
class MissionRecorder:
    def __init__(self, save_dir, cfg):
        ...
        self.pose_id = 0
        self.selected_pose_list = []

    @property
    def is_alive(self):
        return len(self.camera_params_list) < self.max_frames
    ...
    def update_path(self, path, path_length):
        ...
        self.selected_pose_list.append(
            path[-1].detach().cpu().clone().numpy().reshape(-1)
        )
    ...
    def save_path(self):
        if len(self.selected_pose_list) > 0:
            selected_pose_file = f"{self.save_dir}/selected_poses.txt"
            selected_poses = np.stack(self.selected_pose_list, axis=0)
            np.savetxt(selected_pose_file, selected_poses, fmt="%.8f")
            print("----------save selected poses----------")
        else:
            print("selected poses not recorded")
    ...
    def save_baseline_poses(self, save_path, metadata=None, simulator=None):
        if len(self.selected_pose_list) == 0:
            print("no keyframe poses to save for baseline")
            return
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        poses = np.stack(self.selected_pose_list, axis=0).reshape(-1, 4, 4)  # (N, 4, 4)

        views = []
        for i, c2w in enumerate(poses):
            if simulator is not None:
                c2w = simulator.planner_to_json_pose(c2w)
            print("c2www", c2w)
            R = c2w[:3, :3]
            t = c2w[:3, 3]
            M = np.array([
            ...
            quat_xyzw = Rotation.from_matrix(R).as_quat()  # scipy returns xyzw
        ...
        output["quaternion_convention"] = "wxyz"
        output["coordinate_frame"] = "camera_to_world"
        output["up_axis"] = "z"
        output["views"] = views

        with open(save_path, "w") as f:
            json.dump(output, f, indent=2)
        print(f"----------save {len(poses)} baseline poses to {save_path}----------")
