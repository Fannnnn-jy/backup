...
class TwoRobotPickCube(BaseEnv):
    ...
    **Success Conditions:**
    ...
    _sample_video_link = (
        "https://github.com/haosulab/ManiSkill/raw/refs/heads/main/figures/environment_demos/TwoRobotPickCube-v1_rt.mp4"
    )
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        ...
        tcp_to_obj_dist = torch.linalg.norm(self.cube.pose.p - self.left_agent.tcp.pose.p, axis=1)
        ...
        cube_to_other_side_reward = 1 - torch.tanh(
            5 * (torch.max(0.05 - self.cube.pose.p[:, 1], torch.zeros_like(reaching_reward)))
        )
        ...
        tcp_to_obj_dist = torch.linalg.norm(self.cube.pose.p - self.right_agent.tcp.pose.p, axis=1)
        reaching_reward = 1 - torch.tanh(5 * tcp_to_obj_dist)
        ...
        tip_height_reward = 1 - torch.tanh(5 * torch.abs(right_tip_1_height - right_tip_2_height))
        tip_width_reward = 1 - torch.tanh(
            5
            * torch.abs(
                torch.linalg.norm(
                    self.right_agent.finger1_link.pose.p - self.right_agent.finger2_link.pose.p,
                    axis=1,
                )
                - 0.07
            )
        ...
        left_arm_leave_reward = 1 - torch.tanh(5 * (self.left_agent.tcp.pose.p[:, 1] + 0.2).abs())
        ...
        obj_to_goal_dist = torch.linalg.norm(self.goal_site.pose.p - self.right_agent.tcp.pose.p, axis=1)
        place_reward = 1 - torch.tanh(5 * obj_to_goal_dist)
        stage_3_reward = 2 * place_reward

        # return left arm to original position
        left_qpos_reward = 1 - torch.tanh(
            torch.linalg.norm(self.left_agent.robot.get_qpos() - self.left_init_qpos, axis=1)
        )
        ...
        right_static_reward = 1 - torch.tanh(5 * torch.linalg.norm(self.right_agent.robot.get_qvel()[..., :-2], axis=1))
        left_static_reward = 1 - torch.tanh(5 * torch.linalg.norm(self.left_agent.robot.get_qvel()[..., :-2], axis=1))
