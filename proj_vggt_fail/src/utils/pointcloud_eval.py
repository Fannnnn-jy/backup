...
def chamfer_distance(
    ...
    *,
    ...
    return float(float(symmetric_scale) * (mins_a.mean() + mins_b.mean()).item())
...
class EpisodePointCloudEvaluator:
    def __init__(self, cfg: PointCloudEvalConfig) -> None:
        self.cfg = cfg
        self._device = _resolve_device(cfg.device)
        self._model: Optional[VGGT] = None
        self._buffers: Dict[int, List[FrameData]] = {}
        self._episode_counter = 0
        self._debug_dump_done = False
        self._buffer_debug_dumped_envs: set[int] = set()

    def _ensure_model(self) -> VGGT:
        if self._model is not None:
            return self._model
        model = VGGT.from_pretrained(
            self.cfg.model_id,
            local_files_only=self.cfg.model_local_files_only,
            cache_dir=self.cfg.model_cache_dir or None,
        ).to(self._device)
        model.eval()
        self._model = model
        return model
    ...
    def _prepare_vggt_images(self, frames: List[FrameData]) -> torch.Tensor:
        rgbs = [f.rgb for f in frames]
        arr = np.stack(rgbs, axis=0)
        arr = np.clip(arr, 0.0, 1.0)
        tensor = torch.from_numpy(arr).permute(0, 3, 1, 2)
        tensor = _resize_square(tensor, int(self.cfg.vggt_input_size))
    ...
    def _compute_predicted_pointcloud(self, frames: List[FrameData]) -> Dict[str, np.ndarray]:
        ...
        images = self._prepare_vggt_images(frames)
        with torch.inference_mode():
            amp_enabled = bool(self.cfg.use_amp and self._device.type == "cuda")
            dtype = torch.bfloat16 if str(self.cfg.amp_dtype).lower() == "bfloat16" else torch.float16
    ...
    def _resolve_debug_dir(self) -> Path:
        root = str(getattr(self.cfg, "debug_dump_dir", "") or "debug/pointcloud_eval")
        return Path(root)
    ...
    def evaluate_episode(self, env_index: int) -> Optional[Dict[str, float]]:
        ...
        distance_mode = str(self.cfg.distance_mode).lower()
        if distance_mode in ("chamfer", "chamfer_l2", "l2"):
        ...
        elif distance_mode in ("chamfer_sum", "chamfer_noscale"):
            dist = chamfer_distance(
                pred_points_aligned,
                gt_points,
                max_points=int(self.cfg.max_points),
        ...
        elif distance_mode in ("chamfer_sq", "chamfer_l2_sq", "l2_sq"):
            dist = chamfer_distance(
                pred_points_aligned,
                gt_points,
                max_points=int(self.cfg.max_points),
        ...
        elif distance_mode in ("chamfer_sq_sum", "chamfer_sq_noscale"):
            dist = chamfer_distance(
                pred_points_aligned,
                gt_points,
                max_points=int(self.cfg.max_points),
...
class PointCloudEpisodeCallback:
    ...
    def __init__(
        ...
        *,
        ...
        self._global_step = 0
        self._evaluator = EpisodePointCloudEvaluator(cfg) if cfg.enabled else None
