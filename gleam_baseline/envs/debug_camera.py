...
def dump_camera_buffer_stats(
    cam: Any, names: Iterable[str] | None = None, *, note: str = ""
) -> None:
    if cam is None:
        return
    if names is None:
    ...
    for name in names:
        ...
        if arr.ndim == 3:
            ...
            stats = ",".join(
                f"{means[idx]:.4f}/{mins[idx]:.4f}/{maxs[idx]:.4f}"
...
def print_camera_debug(env: Any, step_idx: int, *, note: str = "") -> None:
    ...
    pos_str = "None" if pos_xy is None else f"({pos_xy[0]:.3f},{pos_xy[1]:.3f})"
    cam_pos_str = "None"
