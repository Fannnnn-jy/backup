...
class UnitreeG1UpperBody(BaseAgent):
    ...
    def _after_init(self):
        ...
        self.right_finger_joint_indexes = [
            self.robot.active_joints_map[joint].active_index[0].item() for joint in self.right_finger_joints
        ]
        ...
        self.left_finger_joint_indexes = [
            self.robot.active_joints_map[joint].active_index[0].item() for joint in self.left_finger_joints
        ]
    ...
    def right_hand_dist_to_open_grasp(self):
        ...
        return torch.mean(torch.abs(self.robot.qpos[:, self.right_finger_joint_indexes]), dim=1)

    def left_hand_dist_to_open_grasp(self):
        """compute the distance from the current qpos to a open grasp qpos for the left hand"""
        return torch.mean(torch.abs(self.robot.qpos[:, self.left_finger_joint_indexes]), dim=1)
    ...
    def left_hand_is_grasping(self, object: Actor, min_force=0.5, max_angle=85):
        ...
        """
        l_contact_forces = self.scene.get_pairwise_contact_forces(self.left_hand_finger_link_l_1, object)
        r_contact_forces_1 = self.scene.get_pairwise_contact_forces(self.left_hand_finger_link_r_1, object)
        r_contact_forces_2 = self.scene.get_pairwise_contact_forces(self.left_hand_finger_link_r_2, object)
    ...
    def right_hand_is_grasping(self, object: Actor, min_force=0.5, max_angle=85):
        ...
        Args:
            ...
            max_angle (int, optional): Maximum angle of contact to consider grasping. Defaults to 85.
        """
        l_contact_forces = self.scene.get_pairwise_contact_forces(self.right_hand_finger_link_l_1, object)
        r_contact_forces_1 = self.scene.get_pairwise_contact_forces(self.right_hand_finger_link_r_1, object)
        r_contact_forces_2 = self.scene.get_pairwise_contact_forces(self.right_hand_finger_link_r_2, object)
        lforce = torch.linalg.norm(l_contact_forces, axis=1)
        rforce_1 = torch.linalg.norm(r_contact_forces_1, axis=1)
        rforce_2 = torch.linalg.norm(r_contact_forces_2, axis=1)
...
class UnitreeG1UpperBodyRightArm(UnitreeG1UpperBody):
    ...
    def _controller_configs(self):
        ...
        passive = PassiveControllerConfig(
            [x for x in list(self.robot.active_joints_map.keys()) if x not in self.body_joints],
            damping=self.body_damping,
        )
        return dict(
            pd_joint_pos=dict(body=body_pd_joint_pos, passive=passive, balance_passive_force=True),
