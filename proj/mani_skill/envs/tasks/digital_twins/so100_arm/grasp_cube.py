...
class SO100GraspCubeEnv(BaseDigitalTwinEnv):
    ...
    **Success Conditions:**
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if greenscreen_overlay_path is None:
            logger.warning("No greenscreen overlay path provided, no greenscreen will be used")
    ...
    def _load_lighting(self, options: dict):
        ...
        self.scene.add_directional_light([1, 1, -1], [1, 1, 1], shadow=False, shadow_scale=5, shadow_map_size=2048)
    ...
    def _load_scene(self, options: dict):
        ...
        colors[:, 0] = 1
        frictions = np.ones(self.num_envs) * self.domain_randomization_config.cube_friction_mean
        ...
        if self.domain_randomization:
            ...
            frictions = frictions.clip(*self.domain_randomization_config.cube_friction_bounds)
                            ...
                            for part in render_shape.parts:
                                if (
                                    ...
                                    part.material.set_base_color(
                                        self._batched_episode_rng[i].uniform(low=0.0, high=1.0, size=(3,)).tolist()
                                        + [1]
                                    )
                                else:
                                    part.material.set_base_color(
                                        list(self.domain_randomization_config.robot_color) + [1]
                                    )
    ...
    def sample_camera_poses(self, n: int):
        ...
        if self.domain_randomization:
            ...
            self.base_camera_settings["pos"] = common.to_tensor(self.base_camera_settings["pos"], device=self.device)
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            self.agent.robot.set_pose(Pose.create_from_pq(p=[0, 0, 0], q=euler2quat(0, 0, np.pi / 2)))

            # initialize the cube at a random position and rotation around the z-axis
            spawn_box_pos = self.agent.robot.pose.p + torch.tensor([self.spawn_box_pos[0], self.spawn_box_pos[1], 0])
            xyz = torch.zeros((b, 3))
            xyz[:, :2] = torch.rand((b, 2)) * self.spawn_box_half_size * 2 - self.spawn_box_half_size
    ...
    def evaluate(self):
        ...
        l_contact_forces = self.scene.get_pairwise_contact_forces(self.agent.finger1_link, self.table_scene.table)
        r_contact_forces = self.scene.get_pairwise_contact_forces(self.agent.finger2_link, self.table_scene.table)
