...
class PDJointPosController(BaseController):
    ...
    def _get_joint_limits(self):
        qlimits = self.articulation.get_qlimits()[0, self.active_joint_indices].cpu().numpy()
    ...
    def set_drive_property(self):
        ...
        for i, joint in enumerate(self.joints):
            ...
            joint.set_drive_properties(stiffness[i], damping[i], force_limit=force_limit[i], mode=drive_mode)
    ...
    def reset(self):
        ...
        if self._start_qpos is None:
            ...
            self._start_qpos[self.scene._reset_mask] = self.qpos[self.scene._reset_mask].clone()
        if self._target_qpos is None:
            self._target_qpos = self.qpos.clone()
        else:
            self._target_qpos[self.scene._reset_mask] = self.qpos[self.scene._reset_mask].clone()

    def set_drive_targets(self, targets):
        self.articulation.set_joint_drive_targets(targets, self.joints, self.active_joint_indices)
    ...
    def set_action(self, action: Array):
        ...
        if self.config.use_delta:
            ...
            self._target_qpos = torch.broadcast_to(action, self._start_qpos.shape).clone()
...
class PDJointPosControllerConfig(ControllerConfig):
    ...
    force_limit: float | Sequence[float] = 1e10
    friction: float | Sequence[float] = 0.0
    use_delta: bool = False
    use_target: bool = False
    interpolate: bool = False
    normalize_action: bool = True
    drive_mode: Sequence[DriveMode] | DriveMode = "force"
...
class PDJointPosMimicController(PDJointPosController):
    ...
    def _initialize_joints(self):
        ...
        if len(self.config.mimic) == 0:
            if len(self.config.joint_names) == 2:
                ...
                self.config.mimic = {self.config.joint_names[0]: {"joint": self.config.joint_names[1]}}
            else:
                raise ValueError(
        ...
        for i, (mimic_joint_name, mimic_data) in enumerate(self.config.mimic.items()):
            ...
            assert self.articulation.joints_map[mimic_joint_name].active_index is not None, (
                f"Mimic joint {mimic_joint_name} is not active, cannot be used as a mimic joint in a mimic controller"
            )
            assert self.articulation.joints_map[control_joint_name].active_index is not None, (
            ...
            self.mimic_joint_indices.append(
                torch.argwhere(
                    self.active_joint_indices == self.articulation.joints_map[mimic_joint_name].active_index[0]
                )
            )
            self.mimic_control_joint_indices.append(
                torch.argwhere(
                    self.active_joint_indices == self.articulation.joints_map[control_joint_name].active_index[0]
                )
            )
        ...
        self.mimic_joint_indices = torch.tensor(self.mimic_joint_indices, dtype=torch.int32, device=self.device)
        ...
        self.control_joint_indices = torch.unique(self.mimic_control_joint_indices).to(torch.int32)
    ...
    def set_action(self, action: Array):
        ...
        self._target_qpos[:, self.mimic_joint_indices] = (
            self._target_qpos[:, self.mimic_control_joint_indices] * self._multiplier[None, :] + self._offset[None, :]
        )
        if self.config.interpolate:
            self._step_size = (self._target_qpos - self._start_qpos) / self._sim_steps
        else:
