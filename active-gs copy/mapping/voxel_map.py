...
class VoxelMap:
    ...
    def index_2_xyz(self, indices):
        ...
        dim_max = (self.dim - 1).to(indices.device)
        indices = torch.maximum(indices, torch.zeros_like(indices))
        indices = torch.minimum(indices, dim_max.unsqueeze(0))
        indices_1d = (
    ...
    def xyz_2_index(self, xyz):
        ...
        voxel_indices = torch.floor(relative_positions / self.size).int()
        dim_max = (self.dim - 1).to(voxel_indices.device)
        voxel_indices = torch.maximum(voxel_indices, torch.zeros_like(voxel_indices))
        voxel_indices = torch.minimum(voxel_indices, dim_max)
