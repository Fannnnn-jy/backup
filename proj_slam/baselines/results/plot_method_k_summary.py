...
PLOT_METRICS = [
    ("coverage_mean", "coverage", "Coverage"),
    ("cd_mean", "cd", "Chamfer Distance"),
    ("l2_mean", "l2", "Depth L2"),
]


def load_rows(csv_path: Path) -> dict[str, dict[str, list[float]]]:
    data: dict[str, dict[str, list[float]]] = defaultdict(lambda: {"k": []})

    with csv_path.open(newline="") as f:
        reader = csv.DictReader(f)
        required = {"method", "k", "coverage_mean", "cd_mean", "l2_mean"}
        ...
        for row in reader:
            ...
            method_data.setdefault("k", []).append(int(row["k"]))
            for column_name, short_name, _ in PLOT_METRICS:
                method_data.setdefault(short_name, []).append(float(row[column_name]))
                std_column = f"{short_name}_std"
                if std_column in row and row[std_column] not in ("", None):
                    method_data.setdefault(std_column, []).append(float(row[std_column]))
                else:
                    method_data.setdefault(std_column, []).append(0.0)
...
def load_file_records(input_dir: Path, input_glob: str, dataset_prefix: str) -> list[dict]:
    ...
    for path in sorted(input_dir.glob(input_glob)):
        if path.name.endswith("_cut3r.json") or path.name.endswith("_wint3r.json"):
            continue
...
def build_summary(records: list[dict], resolved_max_k: int, dataset_prefix: str, input_glob: str) -> dict:
    ...
    for record in records:
        ...
        for k in range(1, resolved_max_k + 1):
            ...
            for raw_key, short_name, _ in RAW_METRICS:
...
def plot_metric(ax, data: dict[str, dict[str, list[float]]], metric_key: str, title: str) -> None:
    max_k = max(max(values["k"]) for values in data.values())
    tick_step = 1 if max_k <= 10 else 2

    for method, values in sorted(data.items(), key=lambda item: method_sort_key(item[0])):
        std_key = f"{metric_key}_std"
        means = values[metric_key]
        stds = values.get(std_key, [0.0] * len(means))
        ...
        if any(std > 0.0 for std in stds):
            ax.fill_between(values["k"], lower, upper, color=color, alpha=0.16)
    ax.set_title(title)
    ax.set_xlabel("k")
    ax.set_xlim(1, max_k)
...
def save_combined_figure(data: dict[str, dict[str, list[float]]], output_path: Path) -> None:
    fig, axes = plt.subplots(1, 3, figsize=(16, 4.8), constrained_layout=True)
    for ax, (_, short_name, title) in zip(axes, PLOT_METRICS):
        plot_metric(ax, data, short_name, title)
    axes[0].set_ylabel("Value")
    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper center", ncol=min(4, len(labels)), frameon=False, bbox_to_anchor=(0.5, 1.06))
    fig.suptitle("Method Comparison by k", fontsize=14, y=1.08)
    fig.savefig(output_path, dpi=200, bbox_inches="tight")
    plt.close(fig)


def save_single_metric_figures(data: dict[str, dict[str, list[float]]], output_dir: Path, stem: str) -> list[Path]:
    outputs = []
    for _, short_name, title in PLOT_METRICS:
...
def parse_args() -> argparse.Namespace:
    default_csv = Path(__file__).with_name("method_k_summary_replicacad.csv")
    default_dir = Path(__file__).resolve().parent
    parser = argparse.ArgumentParser(
        description=(
            "Plot method-vs-k line charts from a summary CSV, or aggregate raw JSON results "
            "in baselines/results and generate summary CSV/JSON plus plots."
        )
    )
    parser.add_argument(
        "--csv",
        type=Path,
        default=None,
        help=f"Optional input CSV path for plot-only mode. Example default CSV: {default_csv}",
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=default_dir,
        help=f"Directory containing raw result JSON files. Default: {default_dir}",
    )
    parser.add_argument(
        "--glob",
        default="*.json",
        help="Glob for input result files in raw-JSON mode. Default: *.json",
    )
    parser.add_argument(
        "--dataset-prefix",
        default="replicacad",
        help=(
            "Only include files whose scene_id matches this prefix in raw-JSON mode. "
            "Examples: replicacad, replicacad/apt_0. Use an empty string to disable filtering."
        ),
    )
    parser.add_argument(
        "--max-k",
        type=int,
        default=None,
        help="Target max k in raw-JSON mode. Default: infer from matched files.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=default_dir,
        help=f"Directory for output summary files and images. Default: {default_dir}",
    )
    parser.add_argument(
        "--stem",
        default=None,
        help="Output filename stem. Default: method_k_summary_<dataset-prefix>",
    )
...
def main() -> None:
    ...
    if args.csv is not None:
        ...
        if not records:
            raise FileNotFoundError(
                f"No raw result files matched glob={args.glob!r} with dataset_prefix={args.dataset_prefix!r} in {input_dir}"
            )

        resolved_max_k = args.max_k or max(record["max_views"] for record in records)
        stem = args.stem or f"method_k_summary_{dataset_tag(args.dataset_prefix)}"
    ...
    single_paths = save_single_metric_figures(data, output_dir, stem)
    save_combined_figure(data, combined_path)
