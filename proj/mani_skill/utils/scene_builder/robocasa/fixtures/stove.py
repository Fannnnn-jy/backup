...
class Stove(Fixture):
    ...
    def __init__(self, xml="fixtures/stoves/stove_orig", name="stove", *args, **kwargs):
        ...
        super().__init__(xml=xml, name=name, duplicate_collision_geoms=False, *args, **kwargs)
    ...
    def get_reset_regions(self, env, locs=None):
        ...
        for location in locs:
            site = self.worldbody.find(f"./body/body/site[@name='{self.naming_prefix}burner_{location}_place_site']")
            if site is None:
                site = self.worldbody.find(f"./body/body/site[@name='{self.naming_prefix}burner_on_{location}']")
            if site is None:
                continue
    ...
    def update_state(self, env):
        ...
        for location in STOVE_LOCATIONS:
            ...
            if site is None:
                continue
            site_id = env.sim.model.site_name2id(f"{self.naming_prefix}burner_on_{location}")

            joint = self.knob_joints[location]
            if joint is None:
                env.sim.model.site_rgba[site_id][3] = 0.0
                continue
            joint_id = env.sim.model.joint_name2id(f"{self.naming_prefix}knob_{location}_joint")
