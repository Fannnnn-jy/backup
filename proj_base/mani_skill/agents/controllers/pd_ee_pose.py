...
class PDEEPosController(PDJointPosController):
    ...
    def _initialize_joints(self):
        ...
        if self.config.root_link_name is not None:
            self.root_link = sapien_utils.get_obj_by_name(self.articulation.get_links(), self.config.root_link_name)
        else:
    ...
    def reset(self):
        ...
        if self.config.use_target:
            if self._target_pose is None:
                ...
                self._target_pose.raw_pose[self.scene._reset_mask] = self.ee_pose_at_base.raw_pose[
...
class PDEEPosControllerConfig(ControllerConfig):
    ...
    delta_solver_config: dict = field(default_factory=lambda: dict(type="levenberg_marquardt", alpha=1.0))
    ...
    def compute_target_pose(self, prev_ee_pose_at_base: Pose, action):
        if self.config.use_delta:
            ...
            if "body_translation" in self.config.frame:
                p = prev_ee_pose_at_base.p + quaternion_apply(prev_ee_pose_at_base.q, delta_pose.p)
...
class PDEEPoseControllerConfig(PDEEPosControllerConfig):
    rot_lower: float | Sequence[float] = None
    ...
    rot_upper: float | Sequence[float] = None
