...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Aggregate *_cut3r.json results into summary CSV/JSON files and "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=default_dir,
        help=f"Directory containing *_cut3r.json files. Default: {default_dir}",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--glob",
        default="*_cut3r.json",
        help="Glob for input result files. Default: *_cut3r.json",
    )
    parser.add_argument(
        "--dataset-prefix",
        default="replicacad",
    ...
    parser.add_argument(
        "--stem",
        default=None,
        help="Output filename stem. Default: method_k_summary_<dataset-prefix>_cut3r",
    )
...
def load_file_records(input_dir: Path, input_glob: str, dataset_prefix: str) -> list[dict]:
    ...
    for path in sorted(input_dir.glob(input_glob)):
        ...
        if not scene_id or not method or not isinstance(metrics_by_k, list) or not metrics_by_k:
            raise ValueError(f"Unexpected cut3r result format: {path}")
...
def main() -> None:
    ...
    if not records:
        raise FileNotFoundError(
            f"No cut3r result files matched glob={args.glob!r} with dataset_prefix={args.dataset_prefix!r} in {input_dir}"
        )

    inferred_max_k = max(record["max_views"] for record in records)
    resolved_max_k = args.max_k or inferred_max_k
    stem = args.stem or f"method_k_summary_{dataset_tag(args.dataset_prefix)}_cut3r"
