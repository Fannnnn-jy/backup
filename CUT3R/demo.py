...
def save_pointcloud_as_ply(
    pts3ds, colors, confs, outdir, filename="reconstruction_cut3r.ply", conf_threshold=0.0
):
    """Save a merged point cloud to ASCII PLY."""
    merged_pts = []
    merged_colors = []

    for i, (pts, color) in enumerate(zip(pts3ds, colors)):
        pts_np = pts.detach().cpu().numpy() if torch.is_tensor(pts) else np.asarray(pts)
        color_np = (
            color.detach().cpu().numpy() if torch.is_tensor(color) else np.asarray(color)
        )
        pts_np = pts_np.reshape(-1, 3)
        color_np = color_np.reshape(-1, 3)

        mask = np.isfinite(pts_np).all(axis=1) & np.isfinite(color_np).all(axis=1)
        if confs is not None and i < len(confs):
            conf_np = (
                confs[i].detach().cpu().numpy()
                if torch.is_tensor(confs[i])
                else np.asarray(confs[i])
            )
        ...
        if not np.any(mask):
            continue
        merged_pts.append(pts_np[mask])
        merged_colors.append(color_np[mask])

    if not merged_pts:
        print("No valid points to save in PLY.")
        return None

    merged_pts = np.concatenate(merged_pts, axis=0)
    merged_colors = np.clip(np.concatenate(merged_colors, axis=0) * 255, 0, 255).astype(
        np.uint8
    )
    ...
    with open(ply_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"element vertex {len(merged_pts)}\n")
        ...
        f.write("end_header\n")
        for p, c in zip(merged_pts, merged_colors):
            f.write(f"{p[0]} {p[1]} {p[2]} {int(c[0])} {int(c[1])} {int(c[2])}\n")

    print(f"Saved point cloud PLY to {ply_path} ({len(merged_pts)} points).")
    return ply_path
...
def run_inference(args):
    ...
    save_pointcloud_as_ply(
        ...
        conf,
        args.output_dir,
        filename="reconstruction_cut3r.ply",
        conf_threshold=args.vis_threshold,
    )
