...
def sensor_data_to_pointcloud(observation: dict, sensors: dict[str, BaseSensor]):
    ...
    for (cam_uid, images), (sensor_uid, sensor) in zip(sensor_data.items(), sensors.items()):
        ...
        if isinstance(sensor, Camera):
            ...
            position[..., :3] = position[..., :3] / 1000.0  # convert the raw depth from millimeters to meters
            ...
            if "segmentation" in images:
                cam_pcd["segmentation"] = segmentation.reshape(segmentation.shape[0], -1, 1)
