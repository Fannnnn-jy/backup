...
class BlockRope(nn.Module):
    ...
    def forward(self, x: Tensor, xpos=None, N=None, branch=1, global_=False, kvcache=None, use_cache=False, idx=None) -> Tensor:
        def attn_residual_func(x, kvcache=None):
            if global_ and not use_cache:
                if branch == 1:
                    B,NP,C = x.shape
                    S = N
                    L = NP #S * P
                    ...
                    attn_mask = future_frame.to(x.dtype) * torch.finfo(x.dtype).min 
            ...
            if use_cache:
                S = N
                attn_out, kvcache = self.attn(self.norm1(x), xpos=xpos, attn_mask=attn_mask, use_cache=use_cache, kvcache=kvcache, S=N, idx=idx)
                return self.ls1(attn_out), kvcache
            else:
                attn_out = self.attn(self.norm1(x), xpos=xpos, attn_mask=attn_mask)
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            if use_cache:
                res, kv = attn_residual_func(x, kvcache)
                x = x + res #attn_residual_func(x)
                x = x + ffn_residual_func(x)
