...
@register_env("RoboCasaKitchen-v1", max_episode_steps=100, asset_download_ids=["RoboCasa"])
class RoboCasaKitchenEnv(BaseEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.layout_and_style_ids = [
            (int(l), int(s)) for (l, s) in self.layout_and_style_ids if l not in self.EXCLUDE_LAYOUTS
        ]
    ...
    def _default_sensor_configs(self):
        ...
        return [CameraConfig("base_camera", pose, 128, 128, 60 * np.pi / 180, 0.01, 100)]

    @property
    def _default_human_render_camera_configs(self):
        pose = sapien_utils.look_at([3.0, -7.5, 2.5], [3.0, 0.0, 1.0])
        return CameraConfig("render_camera", pose, 2048, 2048, 60 * np.pi / 180, 0.01, 100)
                    ...
                    for i in range(10):
                        try:
                            object_placements = placement_initializer.sample(
                                placed_objects=self.scene_builder.scene_data[self._scene_idx_to_be_loaded][
                                    "fxtr_placements"
                                ]
                            )
