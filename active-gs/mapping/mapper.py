...
_DEPTH_ANYTHING_MODEL_NAME = "depth-anything/da3mono-large"
_DEPTH_ANYTHING_CACHE_DIR = "/home/ghr/fs/Junyi/model_weights"
...
def get_depth_model():
    ...
    if _DEPTH_ANYTHING_MODEL is None:
        ...
        _DEPTH_ANYTHING_MODEL = DepthAnything3.from_pretrained(
            _DEPTH_ANYTHING_MODEL_NAME,
            cache_dir=_DEPTH_ANYTHING_CACHE_DIR,
        )
        _DEPTH_ANYTHING_MODEL = _DEPTH_ANYTHING_MODEL.to(device=device)
...
def depth_prediction(rgb_np, intrinsic=None):
    ...
    prediction = get_depth_model().inference(img_numpy_list, intrinsics=intrinsic)
...
class IncrementalMapper:
    def __init__(self, cfg, device):
        self.cfg = cfg
        self.device = device
        self.depth_mode = str(getattr(cfg, "depth_mode", "gt")).lower()
        if self.depth_mode not in {"gt", "pred"}:
            raise ValueError(
                f"unsupported mapper.depth_mode={self.depth_mode}, expected 'gt' or 'pred'"
            )
    ...
    def get_new_dataframe(self, i):
        ...
        if self.depth_mode == "pred":
            dataframe = depth2pred(dataframe)
