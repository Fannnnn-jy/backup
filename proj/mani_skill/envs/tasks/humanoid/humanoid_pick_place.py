...
class HumanoidPlaceAppleInBowl(HumanoidPickPlaceEnv):
    ...
    def _default_sensor_configs(self):
        return CameraConfig(
            "base_camera",
            sapien.Pose([0.279123, 0.303438, 1.34794], [0.252428, 0.396735, 0.114442, -0.875091]),
            128,
            128,
            np.pi / 2,
            0.01,
    ...
    def evaluate(self):
        is_obj_placed = torch.linalg.norm(self.bowl.pose.p - self.apple.pose.p, axis=1) <= 0.05
        hand_outside_bowl = self.agent.right_tcp.pose.p[:, 2] > self.bowl.pose.p[:, 2] + 0.125
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        tcp_to_obj_dist = torch.linalg.norm(self.apple.pose.p - self.agent.right_tcp.pose.p, axis=1)
        ...
        obj_to_goal_dist = torch.linalg.norm(
            (self.bowl.pose.p + torch.tensor([0, 0, 0.15], device=self.device)) - self.apple.pose.p,
            axis=1,
        )
        ...
        reward[obj_high_above_bowl] = 4 + place_reward[obj_high_above_bowl] + grasp_release_reward[obj_high_above_bowl]
        reward[info["success"]] = 8 + (place_reward + grasp_release_reward)[info["success"]]
        return reward
...
class UnitreeG1PlaceAppleInBowlEnv(HumanoidPlaceAppleInBowl):
    ...
    **Goal Specification:**
    ...
    _sample_video_link = (
        "https://github.com/haosulab/ManiSkill/raw/main/figures/environment_demos/UnitreeG1PlaceAppleInBowl-v1_rt.mp4"
    )
