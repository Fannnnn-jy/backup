...
def _tile_frames(frames: np.ndarray) -> np.ndarray:
    ...
    for idx in range(num_frames):
        row = idx // grid
        col = idx % grid
        tiled[row * height : (row + 1) * height, col * width : (col + 1) * width] = (
            frames[idx]
        )
    return tiled
...
def _render_topdown(
    ...
    *,
    ...
    if (
        show_error_overlay
        and carved_occupied is not None
        and carved_free is not None
        and obstacle_3d is not None
    ):
        try:
