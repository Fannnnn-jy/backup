...
class Logger:
    ...
    _ROLLOUT_EXTRA_METRICS = {
        "reward_total": ("Reward/reward_total", "Mean step reward total:"),
        "reward_step_penalty": ("Reward/reward_step_penalty", "Mean step penalty:"),
        "reward_coverage_new": ("Reward/reward_coverage_new", "Mean reward coverage new:"),
        "reward_visibility": ("Reward/reward_visibility", "Mean reward visibility:"),
        "reward_overlap": ("Reward/reward_overlap", "Mean reward overlap:"),
        "visible_ratio": ("Coverage/visible_ratio", "Mean visible ratio:"),
        "new_visible_ratio": ("Coverage/new_visible_ratio", "Mean new visible ratio:"),
        "overlap_ratio": ("Coverage/overlap_ratio", "Mean overlap ratio:"),
    }
    def __init__(
        self,
        log_dir: str | None,
        ...
        self.rollout_extra_metrics = {key: [] for key in self._ROLLOUT_EXTRA_METRICS}
    ...
    def process_env_step(
        ...
        if self.log_dir is not None:
            ...
            for key in self._ROLLOUT_EXTRA_METRICS:
                if key not in extras:
                    continue
                value = self._mean_numeric_value(extras[key])
                if value is not None:
                    self.rollout_extra_metrics[key].append(value)
    ...
    @staticmethod
    def _mean_numeric_value(value) -> float | None:
        try:
    ...
    def log(
        ...
        if self.log_dir is not None and not self.disable_logs:
            ...
            rollout_metrics_string = ""
            for key, (tag, label) in self._ROLLOUT_EXTRA_METRICS.items():
                values = self.rollout_extra_metrics.get(key, [])
                if not values:
                    continue
                value = statistics.mean(values)
                self.writer.add_scalar(tag, value, it)
                rollout_metrics_string += f"""{label:>{pad}} {value:.4f}\n"""
            done_reason_string = ""
            episodes_done = len(self.ep_extras)
            if episodes_done > 0:
                final_coverages: list[float] = []
                crash_penalties_per_step: list[float] = []
                ended_by_collision = 0
                ended_by_out_of_bounds = 0
                ended_by_coverage_complete = 0
                ended_by_max_step = 0
                if isinstance(self.env_cfg, dict):
                    crash_penalty = float(self.env_cfg.get("crash_penalty", 0.0))
                else:
                    crash_penalty = float(getattr(self.env_cfg, "crash_penalty", 0.0))
                for ep_info in self.ep_extras:
                    try:
                        final_cov = float(torch.as_tensor(ep_info["final_accumulated_coverage_ratio"]).reshape(-1)[0].item())
                        final_coverages.append(final_cov)
                    except Exception:
                        pass
                    length = float(torch.as_tensor(ep_info["episode_length"]).reshape(-1)[0].item())
                    is_collision = float(torch.as_tensor(ep_info.get("ended_by_collision", 0.0)).reshape(-1)[0].item()) > 0.5
                    is_out = float(torch.as_tensor(ep_info.get("ended_by_out_of_bounds", 0.0)).reshape(-1)[0].item()) > 0.5
                    is_cov = (
                        float(torch.as_tensor(ep_info.get("ended_by_coverage_complete", 0.0)).reshape(-1)[0].item()) > 0.5
                    )
                    is_max = float(torch.as_tensor(ep_info.get("ended_by_max_step", 0.0)).reshape(-1)[0].item()) > 0.5
                    ended_by_collision += int(is_collision)
                    ended_by_out_of_bounds += int(is_out)
                    ended_by_coverage_complete += int(is_cov)
                    ended_by_max_step += int(is_max)
                    if (is_collision or is_out) and length > 0.0:
                        crash_penalties_per_step.append(crash_penalty / length)

                if final_coverages:
                    value = statistics.mean(final_coverages)
                    self.writer.add_scalar("Coverage/final_accumulated_coverage_ratio", value, it)
                    rollout_metrics_string += f"""{'Mean final coverage:':>{pad}} {value:.4f}\n"""

                crash_penalty_mean = statistics.mean(crash_penalties_per_step) if crash_penalties_per_step else 0.0
                self.writer.add_scalar("Reward/reward_crash", crash_penalty_mean, it)
                rollout_metrics_string += f"""{'Mean crash penalty:':>{pad}} {crash_penalty_mean:.4f}\n"""

                self.writer.add_scalar("Done/episodes_done", float(episodes_done), it)
                done_reason_string += f"""{'Episodes done this rollout:':>{pad}} {episodes_done:.0f}\n"""
                metrics = [
                    ("Done/collision_rate", "Episode end collision rate:", ended_by_collision),
                    ("Done/out_of_bounds_rate", "Episode end out-of-bounds rate:", ended_by_out_of_bounds),
                    ("Done/coverage_complete_rate", "Episode end coverage-complete rate:", ended_by_coverage_complete),
                    ("Done/max_steps_rate", "Episode end max-steps rate:", ended_by_max_step),
                ]
                for tag, label, count in metrics:
                    rate = float(count) / float(episodes_done)
                    self.writer.add_scalar(tag, rate, it)
                    done_reason_string += f"""{label:>{pad}} {rate:.4f}\n"""
            else:
                self.writer.add_scalar("Reward/reward_crash", 0.0, it)
                rollout_metrics_string += f"""{'Mean crash penalty:':>{pad}} {0.0:.4f}\n"""
            ...
            if not print_minimal:
                log_string += rollout_metrics_string
                log_string += done_reason_string
            ...
            for values in self.rollout_extra_metrics.values():
                values.clear()
    ...
    def _store_code_state(self) -> None:
        ...
        if self.log_dir is not None and not self.disable_logs:
            ...
            for repository_file_path in self.git_status_repos:
                ...
                with open(diff_file_name, "x", encoding="utf-8", errors="surrogateescape") as f:
