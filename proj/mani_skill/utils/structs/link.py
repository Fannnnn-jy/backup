...
class Link(PhysxRigidBodyComponentStruct[physx.PhysxArticulationLinkComponent]):
    ...
    def merge(cls, links: list[Link], name: str = None):
        ...
        merged_link = Link.create(objs, scene=links[0].scene, scene_idxs=merged_scene_idxs)
        if not has_one_root_link:
    ...
    def generate_mesh(
        self,
        filter: Callable[[physx.PhysxArticulationLinkComponent, sapien.render.RenderShape], bool],
        mesh_name: str,
    ):
        """
    ...
    def index(self) -> torch.Tensor:
        ...
        return torch.tensor([obj.index for obj in self._objs], dtype=torch.int, device=self.device)

    @cached_property
    def is_root(self) -> torch.Tensor:
        return torch.tensor([obj.is_root for obj in self._objs], dtype=torch.bool, device=self.device)
