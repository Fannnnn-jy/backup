...
class BaseDigitalTwinEnv(BaseEnv):
    ...
    _objects_to_remove_from_greenscreen: list[Actor | Link] = []
    ...
    def remove_object_from_greenscreen(self, object: Articulation | Actor | Link):
        ...
        if self.rgb_overlay_mode != "none":
            ...
            for camera_name in self.rgb_overlay_paths.keys():
                ...
                if isinstance(sensor, CameraConfig):
                    ...
                    self._rgb_overlay_images[camera_name] = common.to_tensor(rgb_overlay_img, device=self.device)
    ...
    def _get_obs_sensor_data(self, apply_texture_transforms: bool = True):
        ...
        if (
            ...
            for camera_name in self._rgb_overlay_images.keys():
                ...
                if self._rgb_overlay_images[camera_name].device != obs[camera_name]["rgb"].device:
                    self._rgb_overlay_images[camera_name] = self._rgb_overlay_images[camera_name].to(
