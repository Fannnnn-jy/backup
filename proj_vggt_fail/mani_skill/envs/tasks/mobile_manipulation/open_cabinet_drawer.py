...
class OpenCabinetDrawerEnv(BaseEnv):
    ...
    **Goal Specification:**
    ...
    _sample_video_link = (
        "https://github.com/haosulab/ManiSkill/raw/main/figures/environment_demos/OpenCabinetDrawer-v1_rt.mp4"
    )
    ...
    def _default_sim_config(self):
        return SimConfig(
            spacing=5,
            gpu_memory_config=GPUMemoryConfig(max_rigid_contact_count=2**21, max_rigid_patch_count=2**19),
        )
    ...
    def _load_cabinets(self, joint_types: list[str]):
        ...
        for i, model_id in enumerate(model_ids):
            ...
            cabinet_builder = articulations.get_articulation_builder(self.scene, f"partnet-mobility:{model_id}")
    ...
    def handle_link_positions(self, env_idx: torch.Tensor | None = None):
        if env_idx is None:
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            self.handle_link_goal.set_pose(Pose.create_from_pq(p=self.handle_link_positions(env_idx)))
    ...
    def evaluate(self):
        ...
        link_is_static = (torch.linalg.norm(self.handle_link.angular_velocity, axis=1) <= 1) & (
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        tcp_to_handle_dist = torch.linalg.norm(self.agent.tcp.pose.p - info["handle_link_pos"], axis=1)
        reaching_reward = 1 - torch.tanh(5 * tcp_to_handle_dist)
        amount_to_open_left = torch.div(self.target_qpos - self.handle_link.joint.qpos, self.target_qpos)
