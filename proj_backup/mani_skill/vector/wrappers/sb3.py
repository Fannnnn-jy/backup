...
def select_index_from_dict(data: dict | np.ndarray, i: int):
...
class ManiSkillSB3VectorEnv(SB3VecEnv):
    ...
    def __init__(self, env: BaseEnv):
        ...
        self.episode_returns: torch.Tensor = torch.zeros(self.num_envs, device=self.base_env.device)
        self.episode_lengths: torch.Tensor = torch.zeros(self.num_envs, device=self.base_env.device)
        self.episode_times: torch.Tensor = torch.zeros(self.num_envs, device=self.base_env.device)
        self.total_steps = 0
    ...
    def step_wait(self) -> VecEnvStepReturn:
        vec_obs, rews, terminations, truncations, infos = self._env.step(self.last_actions)
        ...
        for env_idx in range(self.num_envs):
            new_info = {
                "TimeLimit.truncated": truncations[env_idx] and not terminations[env_idx],
                "episode": {
        ...
        for i, done in enumerate(dones):
            if done:
                ...
                new_infos[i]["terminal_observation"] = select_index_from_dict(vec_obs, i)
