...
class Fetch(BaseAgent):
    ...
    urdf_config = dict(
        ...
        link=dict(
            r_gripper_finger_link=dict(material="gripper", patch_radius=0.1, min_patch_radius=0.1),
            l_gripper_finger_link=dict(material="gripper", patch_radius=0.1, min_patch_radius=0.1),
        ),
    )
    ...
    def _after_init(self):
        self.finger1_link: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), "l_gripper_finger_link")
        self.finger2_link: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), "r_gripper_finger_link")
        self.tcp: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), self.ee_link_name)
        ...
        self.torso_lift_link: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), "torso_lift_link")
    ...
    def is_static(self, threshold: float = 0.2, base_threshold: float = 0.05):
        ...
        return torch.all(body_qvel <= threshold, dim=1) & torch.all(base_qvel <= base_threshold, dim=1)
