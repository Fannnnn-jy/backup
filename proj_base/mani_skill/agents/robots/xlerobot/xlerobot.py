...
class Xlerobot(BaseAgent):
    ...
    keyframes = dict(
        rest=Keyframe(
            ...
            qpos=np.array([
                0,
                0,
                0,  # [0,1,2] base: x, y, rotation
                0,
                0,  # [3,4] first values for arm1, arm2
                0,  # [5] head pan
                3.14,
                3.14,  # [6,7] second values for arm1, arm2
                0,  # [8] head tilt
                3.14,
                3.14,  # [9,10] third values for arm1, arm2
                0,
                0,  # [11,12] fourth values for arm1, arm2
                1.57,
                1.57,  # [13,14] fifth values for arm1, arm2
                0,
                0,
            ]),  # [15,16] grippers: Jaw, Jaw_2
        )
    )
    ...
    def _after_init(self):
        # First arm
        self.finger1_link: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), "Fixed_Jaw")
        self.finger2_link: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), "Moving_Jaw")
        self.finger1_tip: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), "Fixed_Jaw_tip")
        self.finger2_tip: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), "Moving_Jaw_tip")
        self.tcp: Link = self.finger1_link

        # Second arm
        self.finger1_link_2: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), "Fixed_Jaw_2")
        self.finger2_link_2: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), "Moving_Jaw_2")
        self.finger1_tip_2: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), "Fixed_Jaw_2_tip")
        self.finger2_tip_2: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), "Moving_Jaw_2_tip")
        self.tcp_2: Link = self.finger1_link_2

        self.base_link: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), "base_link")
        self.top_base_link: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), "top_base_link")
        for link in [self.top_base_link]:
            link.set_collision_group_bit(group=2, bit_idx=FETCH_WHEELS_COLLISION_BIT, bit=1)
        self.base_link.set_collision_group_bit(group=2, bit_idx=FETCH_BASE_COLLISION_BIT, bit=1)

        self.head_camera_link: Link = sapien_utils.get_obj_by_name(self.robot.get_links(), "head_camera_link")
    ...
    def is_static(self, threshold=0.2, base_threshold: float = 0.05):
        qvel = self.robot.get_qvel()[:, 3:-2]  # exclude the base joints
