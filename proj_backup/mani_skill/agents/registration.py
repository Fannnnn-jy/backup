    ...
    def _register_agent(agent_cls: type[T]) -> type[T]:
        if agent_cls.uid in REGISTERED_AGENTS:
            if override:
                ...
                logger.warn(f"Agent {agent_cls.uid} is already registered. Skip registration.")
                return agent_cls

        REGISTERED_AGENTS[agent_cls.uid] = AgentSpec(agent_cls=agent_cls, asset_download_ids=asset_download_ids)
