...
def load_trajectory_from_h5(h5_file: Path) -> tuple[list[dict[str, np.ndarray]], dict[str, Any]]:
    ...
    info = {"action_dim": action_dim, "state_dim": state_dim, "rgb_cameras": rgb_cameras, "metadata": metadata}
...
def create_directory_structure(
    output_dir: str, rgb_cameras: list[str], num_episodes: int, chunks_size: int = DEFAULT_CHUNKS_SIZE
) -> Path:
...
def create_video_from_frames(
    frames: np.ndarray, output_path: Path, fps: int, image_width: int, image_height: int
) -> None:
...
def calculate_statistics(
    all_dataframes: list[pd.DataFrame], all_rgb_data_by_camera: dict[str, list[np.ndarray]], has_state: bool
) -> dict[str, Any]:
...
def main(args: Args):
    ...
    try:
        ...
        for episode_idx, episode_data in enumerate(tqdm(episodes, desc="Processing episodes")):
            ...
            df = process_episode(
                episode_data, episode_idx, info["state_dim"] is not None, args.fps, task_index=0, task_name=task_name
            )
