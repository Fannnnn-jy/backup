...
class FloatingPandaGripper(BaseAgent):
    ...
    keyframes = dict(
        open_facing_down=Keyframe(qpos=[0, 0, 0, 0, np.pi, 0, 0.04, 0.04], pose=sapien.Pose(p=[0, 0, 0.5])),
        open_facing_up=Keyframe(qpos=[0, 0, 0, 0, 0, 0, 0.04, 0.04], pose=sapien.Pose(p=[0, 0, 0.5])),
        open_facing_side=Keyframe(qpos=[0, 0, 0, 0, np.pi / 2, 0, 0.04, 0.04], pose=sapien.Pose(p=[0, 0, 0.5])),
    )
    root_joint_names = [
        "root_x_axis_joint",
        "root_y_axis_joint",
    ...
    def _controller_configs(self):
        ...
        return dict(
            pd_joint_delta_pos=dict(root=arm_pd_joint_delta_pos, gripper=gripper_pd_joint_pos),
