...
def rotation_from_z(z_axis, world_down=None):
    if world_down is None:
        world_down = torch.tensor([0.0, 0.0, -1.0])
    y_axis = world_down

    # check if the Z-axis is collinear with the reference down axis
...
def rotation_from_z_batch(z_axis_batch, world_down=None):
    ...
    if world_down is None:
        world_down = torch.tensor([0.0, 0.0, -1.0])
    y_axis = world_down.to(z_axis_batch.device).expand(
        batch_size, -1
    )
...
def wp2path(
    ...
    num_sample = max(int(num_sample), 1)
    if len(waypoints) == 1:
    ...
    interpolated_rotations = rotation_from_z_batch(interpolated_view_directions, world_down=world_down)
