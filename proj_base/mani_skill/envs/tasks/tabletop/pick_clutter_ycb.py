...
class PickClutterEnv(BaseEnv):
    ...
    def _default_sim_config(self):
        return SimConfig(gpu_memory_config=GPUMemoryConfig(max_rigid_contact_count=2**21, max_rigid_patch_count=2**19))
    ...
    def _sample_target_objects(self):
        ...
        for i in range(self.num_envs):
            ...
            selected_obj_idxs[i] = selected_obj_idxs[i] % len(self.selectable_target_objects[-1])
            target_objects.append(self.selectable_target_objects[-1][selected_obj_idxs[i]])
...
class PickClutterYCBEnv(PickClutterEnv):
    ...
    _sample_video_link = (
        "https://github.com/haosulab/ManiSkill/raw/main/figures/environment_demos/PickClutterYCB-v1_rt.mp4"
    )
