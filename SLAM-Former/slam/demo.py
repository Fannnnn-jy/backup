...
class SLAM:
    ...
    def maybe_to_cpu(self, tensor):
        return tensor.cpu() if self.save_gmem else tensor

    def maybe_to_cuda(self, tensor):
        if tensor is None or tensor.is_cuda:
            return tensor
        return tensor.cuda()

    @staticmethod
    def safe_fps(count, total_time):
        if total_time <= 0:
            return float("nan")
        return float(count) / float(total_time)
    ...
    def backend(self, final=False):
        ...
        map_optimed = self.model.backendT(self.maybe_to_cuda(self.map))
        ...
        if self.map_opt is not None:
            del self.map_opt
            torch.cuda.empty_cache()
        self.map_opt = self.maybe_to_cpu(map_optimed)
    ...
    def map_add(self, token_kf):
        if self.map is None:
            self.map = self.maybe_to_cpu(token_kf) #[tok.cpu() for tok in token_kf]
        else:
            self.map = torch.cat([self.map, self.maybe_to_cpu(token_kf)],axis=0) # S,P,C
    ...
    def terminate(self):
        if self.map is not None and self.nkf % self.backend_every != 0:
            self.signal_backend = True
            self.backend(final=True)

        kf_total = np.sum(self.kf_time)
        ft_total = np.sum(self.times)
        bt_total = np.sum(self.backend_time)
        summary_total = kf_total + ft_total + bt_total
        ...
        print('KFT')
        print('total', kf_total, 'FPS', self.safe_fps(len(self.kf_time), kf_total))
        print('FT')
        print('total', ft_total, 'FPS', self.safe_fps(len(self.times), ft_total))
        print('BT')
        print('total', bt_total, 'FPS', self.safe_fps(len(self.backend_time), bt_total))
        print('Summary')
        print('total', summary_total, 'FPS', self.safe_fps(len(self.kf_time), summary_total))
        if self.map_opt is not None:
            self.save_result(f'{self.outdir}/final', self.map_opt)

    def extract(self, map_all=None):
        result = self.model.extract(self.maybe_to_cuda(map_all))
...
def get_parser():
    ...
    parser.add_argument("--save_gmem", dest="save_gmem", action="store_true", help="Store token maps on CPU to save GPU memory")
    parser.add_argument("--no_save_gmem", dest="save_gmem", action="store_false", help="Keep token maps on GPU to avoid CPU/GPU transfers")
    parser.set_defaults(save_gmem=True)
