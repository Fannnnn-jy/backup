...
class RolloutStorage:
    ...
    class Transition:
        ...
        def __init__(self) -> None:
            ...
            self.visual_feats: torch.Tensor | None = None
    ...
    def add_transition(self, transition: Transition) -> None:
        ...
        if transition.visual_feats is not None:
            if transition.visual_feats.shape[0] != self.num_envs:
                raise ValueError(
                    f"visual_feats leading env dim mismatch: expected {self.num_envs}, "
                    f"got {transition.visual_feats.shape[0]}"
                )
            if self.visual_feats is None:
                self.visual_feats = torch.zeros(
                    self.num_transitions_per_env,
                    *transition.visual_feats.shape,
                    device=self.device,
                    dtype=transition.visual_feats.dtype,
                )
            self.visual_feats[self.step].copy_(transition.visual_feats)
    ...
    def mini_batch_generator(self, num_mini_batches: int, num_epochs: int = 8) -> Generator:
        ...
        visual_feats = self.visual_feats.flatten(0, 1) if self.visual_feats is not None else None
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                visual_feats_batch = visual_feats[batch_idx] if visual_feats is not None else None
