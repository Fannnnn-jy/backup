...
class Keyframe:
    ...
    qpos: Array | None = None
    """the qpos of the robot at this keyframe"""
    qvel: Array | None = None
...
class BaseAgent:
    ...
    def _controller_configs(
        self,
    ) -> dict[str, ControllerConfig | DictControllerConfig]:
    ...
    def _load_articulation(self, initial_pose: sapien.Pose | Pose | None = None):
        """
        Loads the robot articulation
        """

        def build_articulation(scene_idxs: list[int] | None = None):
            loader: URDFLoader | MJCFLoader | None = None
            ...
            assert loader is not None, "No loader found. Provide either path in either urdf_path or mjcf_path"
            ...
            if not os.path.exists(asset_path):
                ...
                if self.uid in assets.DATA_GROUPS or len(assets.DATA_GROUPS[self.uid]) > 0:
                    ...
                    if response:
                        for asset_id in assets.expand_data_group_into_individual_data_source_ids(self.uid):
        ...
        if self.build_separate:
            ...
            for scene_idx in range(self.scene.num_envs):
                ...
                arts.append(robot)
            self.robot = Articulation.merge(arts, name=f"{self.uid}-agent-{self._agent_idx}", merge_links=True)
    ...
    def set_control_mode(self, control_mode: str = None):
        ...
        assert control_mode in self.supported_control_modes, (
            f"{control_mode} not in supported modes: {self.supported_control_modes}"
        )
    ...
    def action_space(self) -> spaces.Space:
        if self._control_mode is None:
            return spaces.Dict({uid: controller.action_space for uid, controller in self.controllers.items()})
        else:
    ...
    def single_action_space(self) -> spaces.Space:
        if self._control_mode is None:
            return spaces.Dict({uid: controller.single_action_space for uid, controller in self.controllers.items()})
        else:
