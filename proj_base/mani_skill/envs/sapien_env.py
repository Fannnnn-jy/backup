...
class BaseEnv(gym.Env):
    ...
    _hidden_objects: list[Actor | Articulation] = []
    ...
    _viewer: sapien.utils.Viewer | None = None

    _sample_video_link: str | None = None
    ...
    def __init__(
        ...
        sensor_configs: dict | None = dict(),
        human_render_camera_configs: dict | None = dict(),
        viewer_camera_configs: dict | None = dict(),
        robot_uids: str | BaseAgent | list[str | BaseAgent] = None,
        ...
        new_sensor_configs: dict[str, BaseSensorConfig] | None = None
    ):
        self._new_sensor_configs = new_sensor_configs
    ...
    def _default_sim_config(self):
        return SimConfig()
    def _load_agent(self, options: dict, initial_agent_poses: sapien.Pose | Pose | None = None, build_separate: bool = False):
        """
    ...
    def _load_scene(self, options: dict):
        ...
        self._sensor_mounts = {}
        for uid, sensor_config in self._new_sensor_configs.items():
            camera_mount_actor = self.scene.create_actor_builder().build_kinematic(f"{uid}_mount")
            self._sensor_mounts[uid] = camera_mount_actor
    ...
    def _setup_sensors(self, options: dict):
        ...
        for uid, sensor_config in self._new_sensor_configs.items():
            sensor_config['mount'] = self._sensor_mounts[uid]
            sensor_config['pose'] = sapien.Pose()  # relative pose to the mount, empty pose means aligned
            self._sensor_configs[uid] = CameraConfig(**sensor_config)
    ...
    # Reset
    # -------------------------------------------------------------------------- #
    def reset(self, seed: None | int | list[int] = None, options: None | dict = None):
    ...
    def _set_episode_rng(self, seed: None | list[int], env_idx: torch.Tensor):
    ...
    def step(self, action: None | np.ndarray | torch.Tensor | dict):
        """
