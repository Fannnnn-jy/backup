...
class ManiSkillScene:
    ...
    def __init__(
        self,
        sub_scenes: list[sapien.Scene] | None = None,
        ...
        self.px: physx.PhysxCpuSystem | physx.PhysxGpuSystem = self.sub_scenes[0].physx_system
        assert all(isinstance(s.physx_system, type(self.px)) for s in self.sub_scenes), (
            "all sub-scenes must use the same simulation backend"
        )
        self.gpu_sim_enabled = True if isinstance(self.px, physx.PhysxGpuSystem) else False
        ...
        self.pairwise_contact_queries: dict[str, physx.PhysxGpuContactPairImpulseQuery] = dict()
        ...
        self.state_dict_registry: StateDictRegistry = StateDictRegistry(actors=dict(), articulations=dict())
    ...
    def remove_actor(self, actor: Actor):
        ...
        if self.gpu_sim_enabled:
            raise NotImplementedError("Cannot remove actors after creating them in GPU sim at the moment")
        else:
    ...
    def remove_articulation(self, articulation: Articulation):
        ...
        if self.gpu_sim_enabled:
            raise NotImplementedError("Cannot remove articulations after creating them in GPU sim at the moment")
        else:
    ...
    def add_camera(
        ...
        near,
        far,
        fovy: float | list | None = None,
        intrinsic: Array | None = None,
        mount: Actor | Link | None = None,
    ) -> RenderCamera:
        """Add's a (mounted) camera to the scene"""
        if SAPIEN_RENDER_SYSTEM == "3.1":
            return self._sapien_31_add_camera(name, pose, width, height, near, far, fovy, intrinsic, mount)
        else:
            return self._sapien_add_camera(name, pose, width, height, near, far, fovy, intrinsic, mount)
                ...
                if self.gpu_sim_enabled:
                    if isinstance(mount, Actor):
                        camera.set_gpu_pose_batch_index(
                            mount._objs[i].find_component_by_type(physx.PhysxRigidBodyComponent).gpu_pose_index
                        )
                    elif isinstance(mount, Link):
                        camera.set_gpu_pose_batch_index(mount._objs[i].gpu_pose_index)
                    else:
                        raise ValueError(f"Tried to mount camera on object of type {mount.__class__}")
    ...
    def _sapien_31_add_camera(
        ...
        for i, scene in enumerate(self.sub_scenes):
            # Create camera component
            camera = RenderCameraComponent(width, height, GlobalShaderPack or get_camera_shader_pack())
            if fovy is not None:
    ...
    def update_render(self, update_sensors: bool = True, update_human_render_cameras: bool = True):
        """
    ...
    def _sapien_update_render(self, update_sensors: bool = True, update_human_render_cameras: bool = True):
        ...
        if self.gpu_sim_enabled:
            if not self.parallel_in_single_scene:
                ...
                if not self._human_render_cameras_initialized and update_human_render_cameras:
    ...
    def _sapien_31_update_render(self, update_sensors: bool = True, update_human_render_cameras: bool = True):
        if self.gpu_sim_enabled:
            if self.render_system_group is None:
    ...
    def get_all_actors(self):
        ...
        """
        return [c.entity for c in self.px.rigid_dynamic_components + self.px.rigid_static_components]

    def get_all_articulations(self):
        """
        Returns list of all physx articulation objects across all sub scenes
        """
        return [c.articulation for c in self.px.articulation_link_components if c.is_root]
    ...
    def create_drive(
        ...
        return Drive.create_from_actors_or_links(self, body0, pose0, body1, pose1, body0._scene_idxs)
    ...
    def add_directional_light(
        ...
        for scene_idx in scene_idxs:
            ...
            light.pose = sapien.Pose(light_position, sapien.math.shortest_rotation([1, 0, 0], direction))
            scene.add_entity(entity)
            if self.parallel_in_single_scene:
    ...
    def add_area_light_for_ray_tracing(
        ...
        lighting_scenes = self.sub_scenes if scene_idxs is None else [self.sub_scenes[i] for i in scene_idxs]
    ...
    def get_pairwise_contact_impulses(self, obj1: Actor | Link, obj2: Actor | Link):
        ...
        if self.gpu_sim_enabled:
            ...
            if rebuild_query:
                ...
                self.pairwise_contact_queries[query_key] = self.px.gpu_create_contact_pair_impulse_query(body_pairs)
    ...
    def get_pairwise_contact_forces(self, obj1: Actor | Link, obj2: Actor | Link):
        """
    ...
    def scene_offsets(self):
        ...
        return torch.tensor(
            np.array([self.px.get_scene_offset(sub_scene) for sub_scene in self.sub_scenes]),
            device=self.device,
        )
    ...
    def scene_offsets_np(self):
        ...
        return np.array([self.px.get_scene_offset(sub_scene) for sub_scene in self.sub_scenes])

    # -------------------------------------------------------------------------- #
    # Simulation state (required for MPC)
    # -------------------------------------------------------------------------- #

    def add_to_state_dict_registry(self, object: Actor | Articulation):
    ...
    def remove_from_state_dict_registry(self, object: Actor | Articulation):
        if isinstance(object, Actor):
            assert object.name in self.state_dict_registry.actors, f"Object {object.name} not in state dict registry"
    ...
    def get_sim_state(self) -> torch.Tensor:
        ...
        for articulation in self.state_dict_registry.articulations.values():
            state_dict["articulations"][articulation.name] = articulation.get_state().clone()
    ...
    def set_sim_state(self, state: dict, env_idx: torch.Tensor = None):
        ...
        if "articulations" in state:
            for art_id, art_state in state["articulations"].items():
                ...
                self.state_dict_registry.articulations[art_id].set_state(art_state, None)
        if env_idx is not None:
    ...
    def _get_all_render_bodies(
        ...
        for actor in self.actors.values():
            ...
            all_render_bodies += [
                (
                    ...
                    entity.find_component_by_type(physx.PhysxRigidDynamicComponent).gpu_pose_index,
                )
        ...
        for articulation in self.articulations.values():
            all_render_bodies += [
                (
                    px_link.entity.find_component_by_type(sapien.render.RenderBodyComponent),
    ...
    def _sapien_setup_gpu_rendering(self):
        ...
        self.render_system_group = sapien.render.RenderSystemGroup([s.render_system for s in self.sub_scenes])
    ...
    def _sapien_31_setup_gpu_rendering(self):
        ...
        for scene in self.sub_scenes:
            ...
            for body in scene.render_system.render_bodies:
                b = body.entity.find_component_by_type(sapien.physx.PhysxRigidBodyComponent)
                if b is None:
                    continue
        ...
        for cameras in self.sensors.values():
            ...
            for c in cameras.camera._render_cameras:
                b = c.entity.find_component_by_type(sapien.physx.PhysxRigidBodyComponent)
                if b is None:
                    continue
        ...
        for name, sensor in sensors.items():
            if isinstance(sensor, Camera):
                batch_renderer = sapien.render.RenderManager(
                    sapien.render.get_shader_pack(sensor.config.shader_config.shader_pack)
                )
    ...
    def get_sensor_images(self, obs: dict[str, Any]) -> dict[str, dict[str, torch.Tensor]]:
    ...
    def get_human_render_camera_images(self, camera_name: str = None) -> dict[str, torch.Tensor]:
        image_data = dict()
        if self.gpu_sim_enabled:
            if self.parallel_in_single_scene:
