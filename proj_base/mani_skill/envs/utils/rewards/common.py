...
def tolerance(x, lower=0.0, upper=0.0, margin=0.0, sigmoid="gaussian", value_at_margin=0.1):
    ...
    if sigmoid in ("cosine", "linear", "quadratic"):
        if not 0 <= value_at_margin < 1:
            raise ValueError(f"`value_at_margin` must be nonnegative and smaller than 1, got {value_at_margin}.")
    else:
        if not 0 < value_at_margin < 1:
            raise ValueError(f"`value_at_margin` must be strictly between 0 and 1, got {value_at_margin}.")
    ...
    if margin == 0:
        ...
        if sigmoid == "gaussian":
            ...
            value = torch.where(in_bounds, torch.tensor(1.0), torch.exp(-0.5 * (d * scale) ** 2))
