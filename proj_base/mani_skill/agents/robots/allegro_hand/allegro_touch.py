...
class AllegroHandRightTouch(AllegroHandRight):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.pair_query: dict[str, tuple[physx.PhysxGpuContactPairImpulseQuery, tuple[int, int, int]]] = dict()
        self.body_query: tuple[physx.PhysxGpuContactBodyImpulseQuery, tuple[int, int, int]] | None = None
    ...
    def get_fsr_obj_impulse(self, obj: Actor = None):
        if self.scene.gpu_sim_enabled:
            ...
            if obj.name not in self.pair_query:
                ...
                obj_bodies = [elem for item in obj._bodies for elem in itertools.repeat(item, 2)]
            ...
            contacts = query.cuda_impulses.torch().clone().reshape((len(self.fsr_links), *contacts_shape))  # [n, 16, 3]
            ...
            contact_forces = [sapien_utils.compute_total_impulse(contact) for contact in sorted_contacts]
    ...
    def get_fsr_impulse(self):
        if self.scene.gpu_sim_enabled:
            ...
            contacts = query.cuda_impulses.torch().clone().reshape(*contacts_shape)  # [n, 16, 3]
            ...
            internal_fsr_links = [link._bodies[0].entity for link in self.fsr_links]
            contacts = self.scene.get_contacts()
            contact_map = sapien_utils.get_cpu_actors_contacts(contacts, internal_fsr_links)
            sorted_contacts = [contact_map[link] for link in internal_fsr_links]
            contact_forces = [sapien_utils.compute_total_impulse(contact) for contact in sorted_contacts]

            contact_impulse = torch.from_numpy(np.stack(contact_forces)[None, ...])  # [1, 16, 3]
