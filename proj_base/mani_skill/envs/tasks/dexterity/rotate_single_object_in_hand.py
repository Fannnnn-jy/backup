...
class RotateSingleObjectInHand(BaseEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not isinstance(difficulty_level, int) or difficulty_level >= 4 or difficulty_level < 0:
            raise ValueError(f"Difficulty level must be a int within 0-3, but get {difficulty_level}")
    ...
    def _load_scene(self, options: dict):
        ...
        if self.difficulty_level == 0:
            ...
            all_model_ids = np.array(list(load_json(ASSET_DIR / "assets/mani_skill2_ycb/info_pick_v0.json").keys()))
            ...
            raise ValueError(f"Difficulty level must be an int within 0-4, but get {self.difficulty_level}")
    ...
    def evaluate(self, **kwargs) -> dict:
        with torch.device(self.device):
            ...
            new_unit_vector -= torch.sum(new_unit_vector * self.rot_dir, dim=-1, keepdim=True) * self.rot_dir
            new_unit_vector = new_unit_vector / torch.linalg.norm(new_unit_vector, dim=-1, keepdim=True)
            angle = torch.acos(torch.clip(torch.sum(new_unit_vector * self.prev_unit_vector, dim=-1), 0, 1))
