...
class Sink(Fixture):
    def __init__(self, scene: ManiSkillScene, xml="fixtures/sink.xml", name="sink", *args, **kwargs):
        self._handle_joint = None
        self._water_site = None
        super().__init__(scene=scene, xml=xml, name=name, duplicate_collision_geoms=False, *args, **kwargs)
    ...
    def get_handle_state(self, env):
        ...
        handle_joint_id = env.sim.model.joint_name2id(f"{self.naming_prefix}handle_joint")
    ...
    def water_site(self):
        ...
        if self._water_site is None:
            self._water_site = self.worldbody.find(f"./body/body/body/site[@name='{self.naming_prefix}water']")
