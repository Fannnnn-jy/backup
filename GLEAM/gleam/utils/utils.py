...
def _bresenham_line_cpu(x0, y0, x1, y1, map_size):
    points = []
    dx = abs(x1 - x0)
    dy = abs(y1 - y0)
...
def bresenham_2d(pts_source, pts_target, map_size):
    ...
    if not PYCUDA_AVAILABLE:
        device = pts_target.device
        source_pt = pts_source.reshape(-1, 2)[0].detach().cpu().tolist()
        target_pts = pts_target.reshape(-1, 2).detach().cpu().tolist()
