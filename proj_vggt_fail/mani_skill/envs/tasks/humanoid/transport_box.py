...
class TransportBoxEnv(BaseEnv):
    ...
    **Success Conditions:**
    ...
    _sample_video_link = (
        "https://github.com/haosulab/ManiSkill/raw/main/figures/environment_demos/UnitreeG1TransportBox-v1_rt.mp4"
    )
    SUPPORTED_ROBOTS = ["unitree_g1_simplified_upper_body_with_head_camera"]
    agent: UnitreeG1UpperBodyWithHeadCamera

    def __init__(self, *args, **kwargs):
        self.init_robot_pose = copy.deepcopy(UnitreeG1UpperBodyWithHeadCamera.keyframes["standing"].pose)
        ...
        super().__init__(*args, robot_uids="unitree_g1_simplified_upper_body_with_head_camera", **kwargs)

    @property
    def _default_sim_config(self):
        return SimConfig(
            gpu_memory_config=GPUMemoryConfig(max_rigid_contact_count=2**22, max_rigid_patch_count=2**21),
    ...
    def _default_sensor_configs(self):
        ...
        return [CameraConfig("base_camera", pose=pose, width=128, height=128, fov=np.pi / 3)]
    ...
    def _load_scene(self, options: dict):
        ...
        model_dir = Path(os.path.join(os.path.dirname(__file__), "../../../utils/scene_builder/table/assets"))
        ...
        visual_file = os.path.join(os.path.dirname(__file__), "assets/cardboard_box/textured.obj")
        builder.add_visual_from_file(
    ...
    def evaluate(self):
        ...
        l_contact_forces = (
            (
                self.scene.get_pairwise_contact_forces(self.agent.robot.links_map["left_five_link"], self.box)
                + self.scene.get_pairwise_contact_forces(self.agent.robot.links_map["left_three_link"], self.box)
                + self.scene.get_pairwise_contact_forces(self.agent.robot.links_map["left_palm_link"], self.box)
            )
            .abs()
            .sum(dim=1)
        )
        r_contact_forces = (
            (
                self.scene.get_pairwise_contact_forces(self.agent.robot.links_map["right_five_link"], self.box)
                + self.scene.get_pairwise_contact_forces(self.agent.robot.links_map["right_three_link"], self.box)
                + self.scene.get_pairwise_contact_forces(self.agent.robot.links_map["right_palm_link"], self.box)
            )
            .abs()
            .sum(dim=1)
        )
        ...
        box_grasped = (
            left_hand_hit_box
            & right_hand_hit_box
            & (self.agent.right_tcp.pose.p[:, 2] < self.box_right_grasp_point.p[:, 2] + 0.04)
            & (self.agent.left_tcp.pose.p[:, 2] < self.box_left_grasp_point.p[:, 2] + 0.04)
        )

        # simply requires box to be resting somewhere on the correct table
        box_at_correct_table_z = (0.751 > self.box.pose.p[:, 2]) & (self.box.pose.p[:, 2] > 0.750)
    ...
    def box_right_grasp_point(self):
        return self.box.pose * Pose.create_from_pq(torch.tensor([-0.165, 0.07, 0.05], device=self.device))
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        ...
        stage_2_reward = (
            ...
            / 4
            + (1 - torch.tanh(3 * torch.linalg.norm(self.agent.left_tcp.pose.p - self.box_left_grasp_point.p, dim=1)))
            / 4
        )
        reward[info["facing_table_with_box"]] = stage_2_reward[info["facing_table_with_box"]]
        # Stage 3 transport box to above the other table, Succeeds if box_at_correct_table_xy
        stage_3_reward = 2 + 1 - torch.tanh((self.agent.robot.qpos[:, 0] - 1.4).abs() / 5)
        ...
        reward[info["box_at_correct_table_xy"]] = stage_4_reward[info["box_at_correct_table_xy"]]
