...
class VGGTRelativeActorCritic(nn.Module):
    ...
    def _adapt_observations(self, obs) -> ObservationAdapterOutput:
        images_raw = obs[self.image_obs_key]
        images, lead_shape = self._prepare_image_sequence(images_raw)
        seq_len = int(images.shape[1])
        valid_mask = None
        if self.valid_mask_obs_key is not None:
            valid_mask = self._prepare_valid_mask(obs[self.valid_mask_obs_key], lead_shape, seq_len, images.device)
    ...
    def _encode_observation(self, obs, *, return_debug: bool = False):
        adapted = self._adapt_observations(obs)

        # visual_feats: [N, S, visual_dim]
        visual_feats = self.feature_extractor(adapted.images)
        # pose_feats: [N, S, pose_dim]
        pose_feats = self.pose_embedder(adapted.pose_mats, adapted.valid_mask)
        # fused_feats: [N, S, fused_dim]
        fused_feats = self.frame_fusion(visual_feats, pose_feats)
        # state_feat: [N, 3 * fused_dim]
        state_feat = self.history_pooling(fused_feats, adapted.valid_mask)

        if not return_debug:
            return state_feat, adapted.lead_shape

        debug = {
            "visual_feats": self._restore_leading(visual_feats, adapted.lead_shape),
            "pose_feats": self._restore_leading(pose_feats, adapted.lead_shape),
            "fused_feats": self._restore_leading(fused_feats, adapted.lead_shape),
            "state_feat": self._restore_leading(state_feat, adapted.lead_shape),
        }
        return state_feat, adapted.lead_shape, debug
    ...
    def forward(self, obs, return_debug: bool = False):
        ...
        action_mean, action_log_std = self.actor_head(state_feat)
        value = self.value_head(state_feat)
        outputs = {
            "action_mean": self._restore_leading(action_mean, lead_shape),
            "action_log_std": self._restore_leading(action_log_std, lead_shape),
            "value": self._restore_leading(value, lead_shape),
        }
        if return_debug:
    ...
    def _update_distribution(self, state_feat: torch.Tensor, lead_shape: tuple[int, ...]) -> None:
        mean, log_std = self.actor_head(state_feat)
        mean = self._restore_leading(mean, lead_shape)
        log_std = self._restore_leading(log_std, lead_shape)
    ...
    def act_inference(self, obs) -> torch.Tensor:
        state_feat, lead_shape = self._encode_observation(obs, return_debug=False)
        mean, _ = self.actor_head(state_feat)
        return self._restore_leading(mean, lead_shape)
