...
class Koch(BaseAgent):
    ...
    urdf_config = dict(
        _materials=dict(gripper=dict(static_friction=0.3, dynamic_friction=0.3, restitution=0.0)),
        link=dict(
        ...
        for link in self.robot.links:
            for i, obj in enumerate(link._objs):
                ...
                if rb_comp is not None:
                    ...
                    meshes_to_modify = [x for x in rb_comp.render_shapes if "chassis" in x.name]
                    ...
                    other_meshes_to_modify = [x for x in rb_comp.render_shapes if "motor" in x.name]
