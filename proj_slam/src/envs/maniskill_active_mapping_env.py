...
from src.utils.camera_pose_update import (
    ...
    build_local_pose_update_debug_defaults,
    normalize_quat_wxyz,
    project_quat_wxyz_to_yaw_pitch_only,
    project_quat_wxyz_to_yaw_only,
)


@dataclass
class EpisodeDiagnosticsState:
    return_sum: float = 0.0
    step_count: int = 0
    sum_visible_ratio: float = 0.0
    sum_new_visible_ratio: float = 0.0
    sum_overlap_ratio: float = 0.0
    count_overlap_in_band: int = 0
    count_zero_new_coverage: int = 0
    count_positive_new_coverage: int = 0
    count_collision_steps: int = 0
    count_out_of_bounds_steps: int = 0
    final_accumulated_coverage_ratio: float = 0.0
    ended_by_collision: bool = False
    ended_by_out_of_bounds: bool = False
    ended_by_max_step: bool = False
    min_overlap_ratio: float = float("inf")
    max_overlap_ratio: float = float("-inf")
    min_new_visible_ratio: float = float("inf")
    max_new_visible_ratio: float = float("-inf")
    overlap_hist: list[int] | None = None
    new_visible_hist: list[int] | None = None
...
class ManiSkillActiveMappingEnv(ActiveMappingEnv):
    ...
    def _setup_diagnostics(self) -> None:
        diag_cfg = self.cfg.diagnostics
        self._diag_step_info_enabled = bool(diag_cfg.step_info_enabled)
        self._diag_episode_summary_enabled = bool(diag_cfg.episode_summary_enabled)
        self._diag_trace_enabled = bool(diag_cfg.enabled and diag_cfg.trace_enabled)
        self._diag_histogram_enabled = bool(diag_cfg.enabled and diag_cfg.histogram_enabled)
        self._diag_action_debug_enabled = bool(diag_cfg.enabled and diag_cfg.step_info_enabled)
        self._diag_histogram_bins = max(1, int(diag_cfg.histogram_bins))
        self._diag_trace_max_steps = max(1, int(diag_cfg.trace_max_steps))
        self._diag_log_every_n_episodes = max(1, int(diag_cfg.log_every_n_episodes))
        self._diag_collect_episode_state = bool(
            self._diag_episode_summary_enabled or self._diag_trace_enabled or self._diag_histogram_enabled
        )
        self._diag_collect_reward_details = bool(
            self._diag_step_info_enabled or self._diag_collect_episode_state
        )
        self._diag_recent_traces: Deque[dict] | None = None
        if self._diag_trace_enabled:
            self._diag_recent_traces = deque(maxlen=max(1, int(diag_cfg.trace_max_episodes)))
        low = float(getattr(self.cfg, "reward_overlap_low", 0.30))
        high = float(getattr(self.cfg, "reward_overlap_high", 0.50))
        if low > high:
            low, high = high, low
        self._diag_overlap_low = low
        self._diag_overlap_high = high
        self._diag_eps = max(float(getattr(self.cfg, "reward_eps", 1e-6)), 1e-12)
        self._diag_episode_state: EpisodeDiagnosticsState | None = None
        self._diag_current_trace: list[tuple] | None = None
        self._diag_current_episode_id = 0
        self._diag_completed_episodes = 0
        self._last_action_pose_debug: dict[str, float | str] | None = None

    def _overlap_in_band(self, overlap_ratio: float) -> bool:
        return self._diag_overlap_low <= overlap_ratio <= self._diag_overlap_high

    def _bucket_index(self, value: float) -> int:
        clipped = min(max(float(value), 0.0), 1.0)
        return min(int(clipped * self._diag_histogram_bins), self._diag_histogram_bins - 1)

    def _current_trace_pose(self) -> tuple[np.ndarray, np.ndarray]:
        position = np.asarray(self._position, dtype=np.float32).reshape(-1)
        if position.size < 3:
            ...
            position = position[:3]
        quat = getattr(self, "_camera_quat_wxyz", None)
        if quat is None:
        ...
        quat = normalize_quat_wxyz(np.asarray(quat, dtype=np.float32))
        return position, quat

    def _reset_episode_reward_stats(self) -> None:
        self._diag_current_episode_id += 1
        if not self._diag_collect_episode_state:
            self._diag_episode_state = None
            self._diag_current_trace = None
            return
        overlap_hist = [0] * self._diag_histogram_bins if self._diag_histogram_enabled else None
        new_visible_hist = [0] * self._diag_histogram_bins if self._diag_histogram_enabled else None
        self._diag_episode_state = EpisodeDiagnosticsState(
            overlap_hist=overlap_hist,
            new_visible_hist=new_visible_hist,
        )
        self._diag_current_trace = [] if self._diag_trace_enabled else None

    def _update_episode_reward_stats(
        self,
        ...
        *,
        ...
        state = self._diag_episode_state
        if state is None:
            return

        reward_total = float(reward_breakdown.get("reward_total", 0.0))
        visible_ratio = float(reward_breakdown.get("visible_ratio", 0.0))
        ...
        accumulated_coverage_ratio = float(reward_breakdown.get("accumulated_coverage_ratio", 0.0))

        state.return_sum += reward_total
        state.step_count += 1
        state.sum_visible_ratio += visible_ratio
        state.sum_new_visible_ratio += new_visible_ratio
        state.sum_overlap_ratio += overlap_ratio
        state.final_accumulated_coverage_ratio = accumulated_coverage_ratio
        if self._overlap_in_band(overlap_ratio):
            state.count_overlap_in_band += 1
        if new_visible_ratio <= self._diag_eps:
            state.count_zero_new_coverage += 1
        if new_visible_ratio > self._diag_eps:
            state.count_positive_new_coverage += 1
        if collision:
            state.count_collision_steps += 1
        if out_of_bounds:
            state.count_out_of_bounds_steps += 1
        state.min_overlap_ratio = min(state.min_overlap_ratio, overlap_ratio)
        state.max_overlap_ratio = max(state.max_overlap_ratio, overlap_ratio)
        state.min_new_visible_ratio = min(state.min_new_visible_ratio, new_visible_ratio)
        state.max_new_visible_ratio = max(state.max_new_visible_ratio, new_visible_ratio)

        if self._diag_histogram_enabled:
            assert state.overlap_hist is not None
            assert state.new_visible_hist is not None
            state.overlap_hist[self._bucket_index(overlap_ratio)] += 1
            state.new_visible_hist[self._bucket_index(new_visible_ratio)] += 1

        if self._diag_trace_enabled and self._diag_current_trace is not None:
            if len(self._diag_current_trace) < self._diag_trace_max_steps:
                position_xyz, camera_quat_wxyz = self._current_trace_pose()
                self._diag_current_trace.append(
                    (
                        int(state.step_count),
                        reward_total,
                        float(reward_breakdown.get("reward_coverage_new", 0.0)),
                        float(reward_breakdown.get("reward_visibility", 0.0)),
                        float(reward_breakdown.get("reward_overlap", 0.0)),
                        visible_ratio,
                        new_visible_ratio,
                        overlap_ratio,
                        ...
                        bool(collision),
                        bool(out_of_bounds),
                        str(crash_reason or "none"),
                        float(position_xyz[0]),
                        float(position_xyz[1]),
                        float(position_xyz[2]),
                        float(camera_quat_wxyz[0]),
                        float(camera_quat_wxyz[1]),
                        float(camera_quat_wxyz[2]),
                        float(camera_quat_wxyz[3]),
                    )
                )

    def _should_emit_episode_summary(self) -> bool:
        if not self._diag_episode_summary_enabled:
            return False
        return (self._diag_completed_episodes % self._diag_log_every_n_episodes) == 0

    def _build_reset_step_info(self, accumulated_coverage_ratio: float) -> dict[str, float | bool | str]:
        info: dict[str, float | bool | str] = {
            ...
            "is_overlap_in_band": False,
            "is_new_coverage_positive": False,
            "is_zero_new_coverage": True,
            "is_empty_visible": True,
        }
        if self._diag_action_debug_enabled:
            info.update(build_local_pose_update_debug_defaults())
        return info
    ...
    def _build_step_info(
        ...
        *,
        ...
        is_first_rewarded_step: bool,
    ) -> dict[str, float | bool | str]:
        visible_ratio = float(reward_breakdown.get("visible_ratio", 0.0))
        new_visible_ratio = float(reward_breakdown.get("new_visible_ratio", 0.0))
        overlap_ratio = float(reward_breakdown.get("overlap_ratio", 0.0))
        info: dict[str, float | bool | str] = dict(reward_breakdown)
        info.update(
            {
                ...
                "crash_reason": str(crash_reason or "none"),
                "episode_step_index": int(episode_step_index),
                "is_first_rewarded_step": bool(is_first_rewarded_step),
                "is_zero_reward_init_frame": False,
                "is_overlap_in_band": bool(self._overlap_in_band(overlap_ratio)),
                "is_new_coverage_positive": bool(new_visible_ratio > self._diag_eps),
                "is_zero_new_coverage": bool(new_visible_ratio <= self._diag_eps),
                "is_empty_visible": bool(visible_ratio <= self._diag_eps),
            }
        )
        if self._diag_action_debug_enabled and self._last_action_pose_debug is not None:
            info.update(self._last_action_pose_debug)
        return info
    ...
    def _build_episode_summary(
        ...
        *,
        ...
        self._diag_completed_episodes += 1
        state.final_accumulated_coverage_ratio = float(accumulated_coverage_ratio)
        state.ended_by_collision = bool(collision)
        state.ended_by_out_of_bounds = bool(out_of_bounds)
        state.ended_by_max_step = bool(truncated and not terminated)
        ended_by_coverage_complete = bool(terminated and not collision and not out_of_bounds and not truncated)

        if self._diag_trace_enabled and self._diag_recent_traces is not None:
            self._diag_recent_traces.append(
                {
                    "episode_id": int(self._diag_current_episode_id),
                    "steps": list(self._diag_current_trace or []),
                }
            )

        if not self._should_emit_episode_summary():
            return None

        step_count = max(0, int(state.step_count))
        denom = float(step_count) if step_count > 0 else 1.0
        summary = {
            "episode_return": float(state.return_sum),
            "episode_length": float(step_count),
            "final_accumulated_coverage_ratio": float(state.final_accumulated_coverage_ratio),
            "mean_visible_ratio": float(state.sum_visible_ratio / denom) if step_count > 0 else 0.0,
            "mean_new_visible_ratio": float(state.sum_new_visible_ratio / denom) if step_count > 0 else 0.0,
            "mean_overlap_ratio": float(state.sum_overlap_ratio / denom) if step_count > 0 else 0.0,
            "fraction_steps_overlap_in_band": float(state.count_overlap_in_band / denom) if step_count > 0 else 0.0,
            "num_zero_new_coverage_steps": float(state.count_zero_new_coverage),
            "num_positive_new_coverage_steps": float(state.count_positive_new_coverage),
            "num_collision_steps": float(state.count_collision_steps),
            "num_out_of_bounds_steps": float(state.count_out_of_bounds_steps),
            "ended_by_collision": float(1.0 if state.ended_by_collision else 0.0),
            "ended_by_out_of_bounds": float(1.0 if state.ended_by_out_of_bounds else 0.0),
            "ended_by_max_step": float(1.0 if state.ended_by_max_step else 0.0),
            "ended_by_coverage_complete": float(1.0 if ended_by_coverage_complete else 0.0),
        }
        if step_count > 0:
            summary["min_overlap_ratio"] = float(state.min_overlap_ratio)
            summary["max_overlap_ratio"] = float(state.max_overlap_ratio)
            summary["min_new_visible_ratio"] = float(state.min_new_visible_ratio)
            summary["max_new_visible_ratio"] = float(state.max_new_visible_ratio)
        else:
            summary["min_overlap_ratio"] = 0.0
            summary["max_overlap_ratio"] = 0.0
            summary["min_new_visible_ratio"] = 0.0
            summary["max_new_visible_ratio"] = 0.0
        if self._diag_histogram_enabled:
            assert state.overlap_hist is not None
            assert state.new_visible_hist is not None
            for idx, count in enumerate(state.overlap_hist):
                summary[f"overlap_hist_bin_{idx}"] = float(count)
            for idx, count in enumerate(state.new_visible_hist):
                summary[f"new_visible_hist_bin_{idx}"] = float(count)
        return summary

    def get_recent_diagnostic_traces(self) -> list[dict]:
        if self._diag_recent_traces is None:
            return []
        return list(self._diag_recent_traces)
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self._position = self._sample_reset_position()
        self._init_position = self._position.copy()
        self._yaw = self._sample_reset_yaw()
        self._camera_quat_wxyz = np.array(_quat_from_yaw(self._yaw), dtype=np.float32)
        ...
        if self._diag_step_info_enabled:
            info.update(self._build_reset_step_info(accumulated_coverage_ratio))
    ...
    def step(self, action: np.ndarray):
        ...
        try:
            ...
            pass
        self._last_action_pose_debug = (
            build_local_pose_update_debug_defaults() if self._diag_action_debug_enabled else None
        )
        ...
        if mode == "xy_cos_sin":
            ...
            if mode == "xyz_delta_quat":
                # This env stores pose as camera-to-world `(p_w, q_wc)`. Under
                # this baseline, `xyz_delta_quat` is interpreted as local
                # camera-frame translation plus local camera-frame delta quaternion.
                current_quat = normalize_quat_wxyz(
                    getattr(self, "_camera_quat_wxyz", np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float32))
                )
                translation_local = action[:3] * self.cfg.action_scale
                next_position, quat, pose_debug = apply_local_camera_delta_pose(
                    prev_position,
                    current_quat,
                    translation_local,
                    action[3:7],
                ...
                if self._diag_action_debug_enabled:
                    self._last_action_pose_debug = pose_debug
            else:
                next_position = prev_position + action[:3] * self.cfg.action_scale
                quat = normalize_quat_wxyz(action[3:7])
            if float(np.linalg.norm(quat)) > 1e-6:
                if bool(getattr(self.cfg, "lock_camera_pitch_roll", False)):
                    quat = project_quat_wxyz_to_yaw_only(quat)
                elif bool(getattr(self.cfg, "lock_camera_roll", False)):
                    quat = project_quat_wxyz_to_yaw_pitch_only(quat)
                self._camera_quat_wxyz = normalize_quat_wxyz(quat)
        ...
        reward, reward_breakdown, accumulated_coverage_ratio = self._compute_reward_breakdown(
            crash_reason=crash_reason if crash else None,
            collect_details=self._diag_collect_reward_details,
        )
        if reward_breakdown is not None and self._diag_collect_episode_state:
        ...
        if accumulated_coverage_ratio >= 1.0 - self._diag_eps:
            terminated = True
        ...
        if self._diag_step_info_enabled and reward_breakdown is not None:
            info.update(
    ...
    def _apply_reward_coverage_dilation(self, mask: np.ndarray) -> np.ndarray:
        dilation = max(0, int(getattr(self.cfg, "reward_coverage_dilation", 0)))
        if dilation <= 0:
            return np.asarray(mask, dtype=bool)
        out = np.asarray(mask, dtype=bool)
        for _ in range(dilation):
            out = self._dilate_mask_3d(out, 3)
        return out
    ...
    def _current_visible_target_voxels(self) -> np.ndarray | None:
        ...
        if masks is not None:
            ...
            if current_visible.shape != target_mask.shape:
                current_visible = np.zeros_like(target_mask, dtype=bool)
        current_visible = self._apply_reward_coverage_dilation(current_visible)
    ...
    def _overlap_band_score(self, overlap_ratio: float) -> float:
        if overlap_ratio < self._diag_overlap_low:
            return -float((self._diag_overlap_low - overlap_ratio) ** 2)
        if overlap_ratio <= self._diag_overlap_high:
            return 1.0
        return -float((overlap_ratio - self._diag_overlap_high) ** 2)
        ...
        *,
    ...
    ) -> tuple[float, dict[str, float] | None, float]:
        ...
        if target_mask is not None:
            ...
            if total_target > 0 and crash_reason is None:
                ...
                coverage_reward = float(getattr(self.cfg, "reward_w_new", 10.0)) * new_visible_ratio
                visibility_reward = float(getattr(self.cfg, "reward_w_visible", 0.1)) * visible_ratio
                overlap_reward = float(getattr(self.cfg, "reward_w_overlap", 1.0)) * self._overlap_band_score(
                    overlap_ratio
                )
        ...
        if not collect_details:
            return float(reward_total), None, float(accumulated_coverage_ratio)
        return (
            float(reward_total),
            {
            ...
            float(accumulated_coverage_ratio),
        )
    ...
    def _update_camera_poses(self) -> None:
        ...
        for angle, cam in zip(self._camera_angles, self._cameras):
            if mode in ("xyz_quat", "xyz_delta_quat") and hasattr(self, "_camera_quat_wxyz"):
            ...
            quat = quat_wxyz
            # SAPIEN pose here is camera-to-world, consistent with voxel carving
            # using `pose_is_camera_to_world=True`.
    ...
    def _sample_reset_position(self) -> np.ndarray:
        position = self._sample_init_pose()
        if not bool(getattr(self.cfg, "init_pose_resample", True)):
            return position

        # For GLB scenes without an explicit init_position, sample a fresh valid
        # reset location instead of always starting from the hard-coded centre.
        should_randomize_glb = self._use_glb_data and getattr(self.cfg, "init_position", None) is None
        if should_randomize_glb:
            candidate = self._sample_valid_position(int(getattr(self.cfg, "init_pose_max_tries", 200)))
            if candidate is not None:
                return candidate
            print("[reset] random GLB init sampling failed; falling back to default init pose.")
    ...
    def _sample_reset_yaw(self) -> float:
        yaw_range = getattr(self.cfg, "maniskill_camera_yaw_range", None)
        if yaw_range is None or len(yaw_range) < 2:
            return math.radians(float(self.cfg.maniskill_camera_yaw_init))
        yaw_low = float(yaw_range[0])
        yaw_high = float(yaw_range[1])
        if yaw_high < yaw_low:
            yaw_low, yaw_high = yaw_high, yaw_low
        if abs(yaw_high - yaw_low) <= 1e-8:
            return math.radians(yaw_low)
        return math.radians(float(self.rng.uniform(yaw_low, yaw_high)))

    def _resolve_reset_sampling_bounds(self) -> Optional[Tuple[float, float, float, float, float, float]]:
        if self._bounds is None:
            return None
        x_min, x_max, y_min, y_max, z_min, z_max = self._bounds
        x_range = getattr(self.cfg, "init_position_x_range", None)
        y_range = getattr(self.cfg, "init_position_y_range", None)
        z_range = getattr(self.cfg, "init_position_z_range", None)
        ...
        if x_max < x_min or y_max < y_min or z_max < z_min:
            raise ValueError(
                "Configured init_position_{x,y,z}_range has no overlap with scene bounds: "
                f"bounds={self._bounds}, x_range={x_range}, y_range={y_range}, z_range={z_range}"
            )
    ...
    def _sample_valid_position(self, max_tries: int) -> Optional[np.ndarray]:
        if max_tries <= 0:
            return None
        sampling_bounds = self._resolve_reset_sampling_bounds()
        if self._obstacle_mask_3d is not None and self._range_gt is not None and self._voxel_size is not None:
            ...
            for _ in range(max_tries):
                ...
                if sampling_bounds is not None:
                    bx_min, bx_max, by_min, by_max, bz_min, bz_max = sampling_bounds
                    if (
                        pos[0] < bx_min
                        or pos[0] > bx_max
                        or pos[1] < by_min
                ...
                if self._is_valid_position(pos):
                    return pos
        if sampling_bounds is not None:
            x_min, x_max, y_min, y_max, z_min, z_max = sampling_bounds
            for _ in range(max_tries):
                pos = np.array(
                    [
