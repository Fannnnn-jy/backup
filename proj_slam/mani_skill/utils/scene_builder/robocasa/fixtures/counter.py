...
class Counter(Fixture):
    ...
    def _set_texture(self, top_texture, base_texture, base_color):
        ...
        for visual_record in self.actor_builder.visual_records:
            ...
            if shortname in self.geom_names and shortname not in self.material_overrides:
                visual_record.material.base_color_texture = sapien.render.RenderTexture2D(
                    filename=self.top_texture if "top" in shortname else self.base_texture,
                    mipmap_levels=1,
                )
                ...
                if "top" in shortname:
                    ...
                    self.actor_builder.add_plane_repeated_visual(
                        pose=sapien.Pose(
                            p=visual_record.pose.p + np.array([0, 0, self.th / 2 + 1e-4]),
    ...
    def _make_counter_with_opening(self, padding):
        ...
        if front_pad < self.overhang:
            raise ValueError(f"Overhang value is too large, must be lower than front padding ({front_pad:.2f})")
        ...
        for side in SIDES:
            ...
            self.actor_builder.add_box_visual(
                name=self.name + f"_top_{side}",
                half_size=half_size,
                pose=sapien.Pose(pos),
                material=self.loader._materials["counter_top"],
            )
            self.actor_builder.add_box_collision(half_size=half_size, pose=sapien.Pose(pos), density=10)
