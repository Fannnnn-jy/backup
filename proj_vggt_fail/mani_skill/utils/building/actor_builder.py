...
class ActorBuilder(SAPIENActorBuilder):
    ...
    def set_scene_idxs(
        self,
        scene_idxs: list[int] | Sequence[int] | torch.Tensor | np.ndarray | None = None,
    ):
        """
        ...
        for r in self.collision_records:
            try:
                if r.type == "plane":
                    ...
                    if self._allow_overlapping_plane_collisions or pose_key not in self._plane_collision_poses:
                    ...
                    shape = physx.PhysxCollisionShapeBox(half_size=r.scale, material=r.material)
                    shapes = [shape]
                elif r.type == "capsule":
    ...
    def build(self, name):
        ...
        assert self.name is not None and self.name != "" and self.name not in self.scene.actors, (
            "built actors in ManiSkill must have unique names and cannot be None or empty strings"
        )

        if self.scene_idxs is not None:
            self.scene_idxs = common.to_tensor(self.scene_idxs, device=self.scene.device).to(torch.int)
        else:
        ...
        if self.scene.parallel_in_single_scene:
            initial_pose_np[:, :3] += self.scene.scene_offsets_np[common.to_numpy(self.scene_idxs)]
        entities = []
        ...
        if self.physx_body_type == "static" and initial_pose_b == 1 and self.scene.gpu_sim_enabled:
            actor.initial_pose = Pose.create(self.initial_pose.raw_pose.repeat(num_actors, 1))
        else:
