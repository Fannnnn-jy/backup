...
class Articulation(BaseStruct[physx.PhysxArticulation]):
    ...
    _net_contact_force_queries: dict[tuple, physx.PhysxGpuContactBodyImpulseQuery] = field(default_factory=dict)
    ...
    def create_from_physx_articulations(
        ...
        all_links_objs: list[list[physx.PhysxArticulationLinkComponent]] = [[] for _ in range(num_links)]
        num_joints = max([len(x.joints) for x in physx_articulations])
        all_joint_objs: list[list[physx.PhysxArticulationJoint]] = [[] for _ in range(num_joints)]

        links_map: dict[str, Link] = dict()
        for articulation in physx_articulations:
            if _process_links:
                assert num_links == len(articulation.links) and num_joints == len(articulation.joints), (
        ...
        for links in all_links_objs:
            ...
            wrapped_link.name = "_".join(links[0].name.replace(self.name, "", 1).split("_")[1:])
        ...
        if _process_links:
            ...
            all_active_joint_names = [x.name for x in physx_articulations[0].get_active_joints()]
            all_joint_names = [x.name for x in physx_articulations[0].joints]
            active_joint_indices = [all_joint_names.index(x) for x in all_active_joint_names]

            joints_map = dict()
            wrapped_joints: list[ArticulationJoint] = []
            for joint_index, joints in enumerate(all_joint_objs):
                try:
                    active_joint_index = all_active_joint_names.index(all_joint_names[joint_index])
                except:
                ...
                wrapped_joint = ArticulationJoint.create(
                    ...
                    joint_index=torch.zeros(len(joints), dtype=torch.int32, device=self.device) + joint_index,
                    active_joint_index=(
                        torch.zeros(len(joints), dtype=torch.int32, device=self.device) + active_joint_index
                        if active_joint_index is not None
                        else None
                    ),
                )
                wrapped_joint.name = "_".join(joints[0].name.replace(self.name, "", 1).split("_")[1:])
            ...
            for joint in self.joints:
                if joint._objs[0].child_link is not None:
                    joint.child_link = self.links_map[
                        "_".join(joint._objs[0].child_link.name.replace(self.name, "", 1).split("_")[1:])
                    ]
                    joint.child_link.joint = joint
                if joint._objs[0].parent_link is not None:
                    joint.parent_link = self.links_map[
                        "_".join(joint._objs[0].parent_link.name.replace(self.name, "", 1).split("_")[1:])
                    ]
        return self
    ...
    def get_first_collision_mesh(self, to_world_frame: bool = True) -> trimesh.Trimesh | None:
        """
    ...
    ) -> list[trimesh.Trimesh] | trimesh.Trimesh:
        """
    ...
    def get_visual_meshes(self, to_world_frame: bool = True, first_only: bool = False) -> list[trimesh.Trimesh]:
        """
        ...
        for i, art in enumerate(self._objs):
            ...
            for link in art.links:
                render_shapes = []
                rb_comp = link.entity.find_component_by_type(sapien.render.RenderBodyComponent)
    ...
    def get_net_contact_impulses(self, link_names: list[str] | tuple[str]):
        ...
        if self.scene.gpu_sim_enabled:
            if tuple(link_names) not in self._net_contact_force_queries:
                ...
                self._net_contact_force_queries[tuple(link_names)] = self.px.gpu_create_contact_body_impulse_query(
                    bodies
                )
            query = self._net_contact_force_queries[tuple(link_names)]
            self.px.gpu_query_contact_body_impulses(query)
            return query.cuda_impulses.torch().clone().reshape(len(link_names), -1, 3).transpose(1, 0)
        else:
            ...
            for contact in contacts:
                if contact.bodies[0].entity in included_links:
                    articulation_contacts[contact.bodies[0].entity.name].append((contact, True))
                elif contact.bodies[1].entity in included_links:
                    articulation_contacts[contact.bodies[1].entity.name].append((contact, False))
    ...
    def get_net_contact_forces(self, link_names: list[str] | tuple[str]):
    ...
    def get_joint_target_indices(self, joint_indices: Array | list[int] | list[ArticulationJoint]):
        """
    ...
    def drive_targets(self):
        ...
        """
        if self.scene.gpu_sim_enabled:
            return self.px.cuda_articulation_target_qpos.torch()[self.get_joint_target_indices(self.active_joints)]
        else:
    ...
    def drive_velocities(self):
        ...
        """
        if self.scene.gpu_sim_enabled:
            return self.px.cuda_articulation_target_qvel.torch()[self.get_joint_target_indices(self.active_joints)]
        else:
            return torch.cat([x.drive_velocity_target for x in self.active_joints], dim=-1)
    ...
    def compute_passive_force(self, *args, **kwargs):
        if self.scene.gpu_sim_enabled:
            raise NotImplementedError("Passive force computation is currently not supported in GPU PhysX")
        else:
    ...
    def get_link_incoming_joint_forces(self):
        ...
        if self.scene.gpu_sim_enabled:
            ...
            return self.px.cuda_articulation_link_incoming_joint_forces.torch()[self._data_index, :]
        else:
            return torch.from_numpy(self._objs[0].get_link_incoming_joint_forces()[None, :])
    ...
    @pose.setter
    def pose(self, arg1: Pose | sapien.Pose) -> None:
        self.root_pose = arg1

    @property
    def qacc(self):
        if self.scene.gpu_sim_enabled:
            return self.px.cuda_articulation_qacc.torch()[self._data_index, : self.max_dof]
        else:
    ...
    def qf(self):
        if self.scene.gpu_sim_enabled:
            return self.px.cuda_articulation_qf.torch()[self._data_index, : self.max_dof]
        else:
    ...
    def qlimits(self):
        padded_qlimits = np.array([
            np.concatenate([obj.qlimits, np.zeros((self.max_dof - obj.dof, 2))]) for obj in self._objs
        ])
    ...
    def qpos(self):
        if self.scene.gpu_sim_enabled:
            ...
            return self.px.cuda_articulation_qpos.torch()[self._data_index, : self.max_dof]
        else:
    ...
    def qvel(self):
        if self.scene.gpu_sim_enabled:
            return self.px.cuda_articulation_qvel.torch()[self._data_index, : self.max_dof]
        else:
    ...
    @root_pose.setter
    def root_pose(self, arg1: Pose | sapien.Pose):
        self.root.pose = arg1
    ...
    def create_pinocchio_model(self):
        ...
        if self.scene.gpu_sim_enabled:
            raise NotImplementedError("Cannot create a pinocchio model when GPU is enabled.")
        else:
    ...
    def set_joint_drive_targets(
        self,
        targets: Array,
        joints: list[ArticulationJoint] | None = None,
