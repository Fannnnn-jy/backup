...
For a custom textured `.glb` scene, create/edit a scene config (e.g. `config/scene/glb/custom.yaml`) and run:
```
python main.py planner=confidence scene=glb/custom use_gui=false
```
`scene_id` should point to your `.glb` file. `mesh_path` can be left `null` for `.glb` scenes.

Selected keyframe poses (NBV poses) are exported to:
```
experiments/<exp_id>/<scene_name>/<planner>/<run_id>/selected_poses.txt
```
Each line is one `4x4` camera-to-world pose flattened to 16 values.
