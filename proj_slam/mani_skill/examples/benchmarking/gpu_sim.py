...
def main(args: Args):
    ...
    if not args.cpu_sim:
        ...
        env = (
            AsyncVectorEnv(
                [make_env() for _ in range(num_envs)], context="forkserver" if sys.platform == "darwin" else None
            )
            if args.num_envs > 1
            else make_env()()
        )
    ...
    if args.save_results:
        ...
        try:
            assert args.save_video == False, "Saving video slows down speed a lot and it will distort results"
