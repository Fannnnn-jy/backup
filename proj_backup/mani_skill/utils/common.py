...
def _batch(array: Array | Sequence):
    ...
    if isinstance(array, list):
        if len(array) == 1:
            return [array]
    if isinstance(array, float) or isinstance(array, int) or isinstance(array, bool) or isinstance(array, np.bool_):
        return np.array([[array]])
    return array


def batch(*args: tuple[Array | Sequence]):
...
def append_dict_array(x1: dict | Sequence | Array, x2: dict | Sequence | Array):
...
def index_dict_array(x1, idx: int | slice, inplace=True):
    """Indexes every array in x1 with slice and returns result."""
    if isinstance(x1, np.ndarray) or isinstance(x1, list) or isinstance(x1, torch.Tensor):
        return x1[idx]
    elif isinstance(x1, dict):
        if inplace:
            for k in x1.keys():
...
def flatten_state_dict(state_dict: dict, use_torch=False, device: Device | None = None) -> Array:
...
def _unbatch(array: Array | Sequence):
    if isinstance(array, (dict)):
...
def unbatch(*args: tuple[Array | Sequence]):
...
def _to_numpy(array: Array | Sequence) -> np.ndarray:
    if isinstance(array, (dict)):
...
def to_numpy(array: Array | Sequence, dtype=None) -> np.ndarray:
