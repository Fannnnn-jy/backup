...
def make_camera_rectangular_prism(n, scale=[0.1, 0.1, 0.1], center=[0, 0, 0], theta=0, device=None) -> torch.Tensor:
    """
    Args:
...
def noised_look_at(eye, target, look_at_noise=1e-2, view_axis_rot_noise=2e-1, device=None) -> Pose:
    """
    Args:
    ...
    angles = torch.normal(torch.zeros(noised_targets.shape[0], device=device), std=view_axis_rot_noise)
