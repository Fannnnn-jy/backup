...
class AntEnv(BaseEnv):
    ...
    def get_vels(self):
        ...
        angvels = torch.stack([link.get_angular_velocity() for link in self.active_links], dim=1)  # (num_links, b, 3)
        linvels = torch.stack([link.get_linear_velocity() for link in self.active_links], dim=1)  # (num_links, b, 3)
    ...
    def control_rew(self, action: Array):
        return rewards.tolerance(action, margin=1, value_at_margin=0, sigmoid="quadratic").mean(dim=-1).view(-1)
