...
class BatchedRNG(np.random.RandomState):
    ...
    def __setitem__(
        self,
        idx: int | list[int] | np.ndarray,
        value: np.random.RandomState | list[np.random.RandomState],
    ):
        idx = common.to_numpy(idx)
        if np.iterable(idx):
    ...
    def __getattribute__(self, item):
        ...
        if callable(getattr(self.rngs[0], item)):
            ...
            def method(*args, **kwargs):
                return np.array([object.__getattribute__(rng, item)(*args, **kwargs) for rng in self.rngs])
