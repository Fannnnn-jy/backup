...
@dataclass
class ActiveMappingDiagnosticsConfig:
    enabled: bool = False
    step_info_enabled: bool = True
    episode_summary_enabled: bool = True
    trace_enabled: bool = False
    trace_max_episodes: int = 8
    trace_max_steps: int = 32
...
class ActiveMappingConfig:
    ...
    glb_voxel_inner_surface: bool = True  # 只保留室内可见内表面体素（从种子点flood-fill）
    glb_voxel_interior_seed: Tuple[float, float, float] = (0.0, 0.0, 0.5)  # 室内已知free点（Z-up坐标）
    reward_coverage_dilation: int = 0
...
class ActiveMappingEnv(gym.Env):
    ...
    def __init__(self, cfg: ActiveMappingConfig, render_mode: Optional[str] = None):
        ...
        if diagnostics_cfg is None:
            cfg.diagnostics = ActiveMappingDiagnosticsConfig()
        elif isinstance(diagnostics_cfg, dict):
            cfg.diagnostics = ActiveMappingDiagnosticsConfig(**diagnostics_cfg)
        self.cfg = cfg
    ...
    def _save_glb_voxel_cache(
        ...
        try:
            with h5py.File(str(cache_path), "w") as f:
                ...
                f.attrs["inner_surface"] = bool(self.cfg.glb_voxel_inner_surface)
            print(f"[glb] saved voxel cache: {cache_path}")
        except Exception as exc:
            print(f"[glb] failed to write voxel cache: {exc}")

    @staticmethod
    def _extract_inner_surface(
        grid: np.ndarray,
        origin: np.ndarray,
        pitch: float,
        seed_world: Tuple[float, float, float],
    ) -> np.ndarray:
        """Extract only the interior-visible surface from a full occupancy grid.

        1. Convert *seed_world* (a known free interior point) to voxel indices.
        2. BFS flood-fill through free (unoccupied) voxels using 6-connectivity
           to discover all interior free space reachable from the seed.
        3. Inner surface = occupied voxels that are 6-adjacent to at least one
           reachable free voxel.

        Returns a new boolean grid of the same shape containing only the inner
        surface voxels.
        """
        from collections import deque

        gx, gy, gz = grid.shape
        seed_idx = np.floor((np.asarray(seed_world, dtype=np.float32) - origin) / pitch).astype(np.int64)
        si, sj, sk = int(seed_idx[0]), int(seed_idx[1]), int(seed_idx[2])

        # Clamp to grid bounds
        si = max(0, min(si, gx - 1))
        sj = max(0, min(sj, gy - 1))
        sk = max(0, min(sk, gz - 1))

        if grid[si, sj, sk]:
            # Seed lands on an occupied voxel; search for nearest free voxel
            print(f"[glb] inner_surface: seed ({si},{sj},{sk}) is occupied, searching nearest free voxel...")
            found = False
            for radius in range(1, max(gx, gy, gz)):
                for di in range(-radius, radius + 1):
                    for dj in range(-radius, radius + 1):
                        for dk in range(-radius, radius + 1):
                            if abs(di) != radius and abs(dj) != radius and abs(dk) != radius:
                                continue
                            ni, nj, nk = si + di, sj + dj, sk + dk
                            if 0 <= ni < gx and 0 <= nj < gy and 0 <= nk < gz and not grid[ni, nj, nk]:
                                si, sj, sk = ni, nj, nk
                                found = True
                                break
                        if found:
                ...
                if found:
                    break
            if not found:
                print("[glb] inner_surface: no free voxel found near seed; returning full grid")
                return grid.copy()

        print(f"[glb] inner_surface: flood-fill from voxel ({si},{sj},{sk}), grid {grid.shape}")

        # BFS flood-fill through free voxels (6-connectivity)
        reachable = np.zeros_like(grid, dtype=bool)
        reachable[si, sj, sk] = True
        queue = deque()
        ...
        while queue:
            ...
            for di, dj, dk in neighbors_6:
                ...
                if 0 <= ni < gx and 0 <= nj < gy and 0 <= nk < gz:
                    if not reachable[ni, nj, nk] and not grid[ni, nj, nk]:
                        reachable[ni, nj, nk] = True
                        queue.append((ni, nj, nk))

        n_reachable = int(reachable.sum())
        print(f"[glb] inner_surface: reachable free voxels = {n_reachable}")

        # Inner surface = occupied voxels adjacent to reachable free voxels
        ...
        reachable_dilated = binary_dilation(reachable, structure=struct)

        inner_surface = grid & reachable_dilated
        n_orig = int(grid.sum())
        n_inner = int(inner_surface.sum())
        print(f"[glb] inner_surface: {n_orig} occupied -> {n_inner} inner surface voxels")
    ...
    def _build_glb_voxel_cache(self, glb_path: Path, cache_path: Path) -> Tuple[np.ndarray, np.ndarray, float]:
        ...
        if self.cfg.glb_voxel_inner_surface:
            grid = self._extract_inner_surface(
                grid, origin, pitch, self.cfg.glb_voxel_interior_seed,
            )
        ...
        if cache_path.exists() and h5py is not None:
            ...
            try:
                with h5py.File(str(cache_path), "r") as f:
                    ...
                    voxel_pitch_attr = attrs.get("voxel_pitch_arg", None)
                    if voxel_pitch_attr is not None and float(voxel_pitch_attr) != float(self.cfg.glb_voxel_pitch):
                        raise ValueError("voxel pitch mismatch.")
                    voxel_max_dim_attr = attrs.get("voxel_max_dim", None)
                    if voxel_max_dim_attr is not None and int(voxel_max_dim_attr) != int(self.cfg.glb_voxel_max_dim):
                        raise ValueError("voxel max dim mismatch.")
                    inner_surface_attr = attrs.get("inner_surface", None)
                    if inner_surface_attr is not None and bool(inner_surface_attr) != bool(self.cfg.glb_voxel_inner_surface):
                        raise ValueError("inner_surface flag mismatch.")
