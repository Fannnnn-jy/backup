...
class CartPoleRobot(BaseAgent):
    ...
    def _controller_configs(self):
        ...
        return dict(pd_joint_delta_pos=dict(slider=pd_joint_delta_pos, rest=rest, balance_passive_force=False))
...
class CartpoleEnv(BaseEnv):
    ...
    def _load_scene(self, options: dict):
        ...
        self.wall.add_box_visual(
            ...
            material=sapien.render.RenderMaterial(base_color=np.array([0.3, 0.3, 0.3, 1])),
        )
    ...
    def compute_dense_reward(self, obs: Any, action: Array, info: dict):
        cart_pos = self.agent.robot.links_map["cart"].pose.p[:, 0]  # (B, ), we only care about x position
        centered = rewards.tolerance(cart_pos, margin=2)
        centered = (1 + centered) / 2  # (B, )

        small_control = rewards.tolerance(action, margin=1, value_at_margin=0, sigmoid="quadratic")[:, 0]
...
class CartpoleBalanceEnv(CartpoleEnv):
    ...
    **Fail Conditions:**
    ...
    _sample_video_link = (
        "https://github.com/haosulab/ManiSkill/raw/main/figures/environment_demos/MS-CartpoleBalance-v1_rt.mp4"
    )
