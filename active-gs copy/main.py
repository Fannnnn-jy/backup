...
def main(cfg):
    ...
    if not cfg.debug and mission_recorder is not None:
        scene_tag = cfg.scene.scene_name.replace("/", "__")
        run_id_str = f"run_{int(cfg.experiment.run_id):03d}"
        pose_dir = os.path.join(cfg.experiment.pose_output_dir, cfg.planner.planner_name)
        pose_path = os.path.join(pose_dir, f"{scene_tag}_{run_id_str}.json")
        metadata = {
            "scene_id": cfg.scene.scene_name,
            "method": cfg.planner.planner_name,
            "run_id": run_id_str,
