...
STANDARD_GLB_VOXEL_PITCH = -1  # ≤0 → use adaptive: max(scene_span) / STANDARD_GLB_VOXEL_MAX_DIM
...
def _build_glb_voxel_cache(
    ...
    *,
    ...
    print(f"[voxelize] loading GLB: {glb_path}")
    t0 = time.perf_counter()
    mesh = _load_glb_mesh_for_voxels(glb_path, y_up=y_up, scale=scale)
    print(f"[voxelize] GLB loaded in {time.perf_counter() - t0:.1f}s  "
    ...
    expected_cells = tuple(int(s / pitch) + 1 for s in size)
    total_cells = expected_cells[0] * expected_cells[1] * expected_cells[2]
    print(f"[voxelize] bounds={bounds[0].tolist()} → {bounds[1].tolist()}")
    print(f"[voxelize] scene size={size.tolist()}")
    print(f"[voxelize] pitch={pitch:.5f}  expected grid={expected_cells}  total={total_cells:,} voxels")
    print(f"[voxelize] starting mesh.voxelized() ...")
    t1 = time.perf_counter()
    vox = mesh.voxelized(pitch)
    print(f"[voxelize] voxelized in {time.perf_counter() - t1:.1f}s")
    ...
    print(f"[voxelize] grid shape={grid.shape}  occupied={occupied:,} ({100*occupied/grid.size:.1f}%)")
    print(f"[voxelize] saving cache: {cache_path}")
    ...
    print(f"[voxelize] total build time: {t_total:.1f}s")
