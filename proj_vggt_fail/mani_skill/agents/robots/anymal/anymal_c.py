...
class ANYmalC(BaseAgent):
    ...
    urdf_config = dict(
        _materials=dict(foot=dict(static_friction=2.0, dynamic_friction=2.0, restitution=0.0)),
        link=dict(
    ...
    keyframes = dict(
        standing=Keyframe(
            ...
            qpos=np.array([0.03, -0.03, 0.03, -0.03, 0.4, 0.4, -0.4, -0.4, -0.8, -0.8, 0.8, 0.8]),
        )
    )
    ...
    def is_standing(self, ground_height=0):
        ...
        angle_diff = torch.arccos(2 * (inner_prod**2) - 1)  # computes an angle between 2 quaternions
        # about 20 degrees
