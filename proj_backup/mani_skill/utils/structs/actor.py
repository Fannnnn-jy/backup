...
class Actor(PhysxRigidDynamicComponentStruct[sapien.Entity]):
    ...
    def create_from_entities(
        ...
        bodies = [ent.find_component_by_type(physx.PhysxRigidDynamicComponent) for ent in entities]
        ...
        if bodies[0] is not None:
            ...
            bodies = [ent.find_component_by_type(physx.PhysxRigidStaticComponent) for ent in entities]
        return cls(
            _objs=entities,
            scene=scene,
            ...
            _bodies=bodies,
            _body_data_name=("cuda_rigid_body_data" if isinstance(scene.px, physx.PhysxGpuSystem) else None),
            name=shared_name,
        )
    ...
    def has_collision_shapes(self):
        ...
        return len(self._objs[0].find_component_by_type(physx.PhysxRigidDynamicComponent).collision_shapes) > 0
    ...
    def hide_visual(self):
        ...
        if self.scene.gpu_sim_enabled:
            ...
            for obj in self._objs:
                obj.find_component_by_type(sapien.render.RenderBodyComponent).visibility = 0
    ...
    def show_visual(self):
        ...
        if self.scene.gpu_sim_enabled:
            ...
            for obj in self._objs:
                obj.find_component_by_type(sapien.render.RenderBodyComponent).visibility = 1
    ...
    def set_collision_group_bit(self, group: int, bit_idx: int, bit: int | bool):
    ...
    def get_collision_meshes(
        ...
        """
        assert not self.merged, "Currently you cannot fetch collision meshes of merged actors"
    ...
    def apply_force(self, force: Array):
        ...
        if self.scene.gpu_sim_enabled:
            self.px.cuda_rigid_body_force.torch()[self._body_data_index, :3] = common.to_tensor(
                force, device=self.device
            )
            self.px.gpu_apply_rigid_dynamic_force()
        else:
            for body in self._bodies:
                body.add_force_at_point(force=force, point=(body.pose * body.cmass_local_pose).p)
    ...
    @pose.setter
    def pose(self, arg1: Pose | sapien.Pose | Array) -> None:
        if self.scene.gpu_sim_enabled:
            ...
            if self.scene.parallel_in_single_scene:
                ...
                new_xyzs = arg1[:, :3] + self.scene.scene_offsets[self._scene_idxs[mask]]
    ...
    def set_pose(self, arg1: Pose | sapien.Pose) -> None:
        self.pose = arg1
