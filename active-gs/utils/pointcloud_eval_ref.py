...
- `episode_XXXX_env_YY/`
  ...
  - `predictions/`: VGGT outputs and derived point clouds.
    ...
    - `pred_points.npy` / `pred_points.ply`: aligned predicted point cloud.
...
class EpisodePointCloudEvaluator:
    ...
    def _dump_debug_artifacts(
        ...
        *,
        ...
        np.save(str(pred_dir / "pred_points.npy"), np.asarray(pred_points, dtype=np.float32))
        ...
        _write_xyz_ply(pred_points, pred_dir / "pred_points.ply")
    ...
    def evaluate_episode(self, env_index: int) -> Optional[Dict[str, float]]:
        ...
        pred_points_aligned = self._align_pred_to_gt(pred_points, pred_extr, frames)
