...
def parse_args():
    ...
    parser.add_argument(
        "-f", "--files", nargs="+", required=True, help="Paths to the benchmark result files to be plotted"
    )
    return parser.parse_args()


COLOR_PALLETE = ["#e02b35", "#59a89c", "#4190F1", "#a559aa#f0c571"]
COLOR_MAP = {"ManiSkill3": "#e02b35", "Isaac Lab": "#59a89c"}
        ...
        if annotate_label is not None:
            for j, (x_val, y_val, annotate_data) in enumerate(zip(xs, df[yname], df[annotate_label])):
                if "gpu_mem_use" in annotate_label:
                    ax.annotate(
                        f"{annotate_data / (1024 * 1024 * 1024):0.1f} GB",
                        (x_val, y_val),
    ...
    for i, (exp_name, df) in enumerate(data.items()):
        ...
        if annotate_label is not None:
            for j, (x, y) in enumerate(zip(df[xname], df[yname])):
                if "gpu_mem_use" in annotate_label:
                    ax.annotate(
                        f"{df[annotate_label].iloc[j] / (1024 * 1024 * 1024):0.1f} GB",
                        (x, y),
                        textcoords="offset points",
                        xytext=(0, 5),
                        ha="center",
...
def main(args):
    ...
    root_save_path = (
        f"benchmark_results/{'_'.join([os.path.basename(file).split('.')[0] for file in args.files])}/{args.env_id}"
    )
        ...
        for cam_size in cam_sizes:
            ...
            draw_bar_plot_envs_vs_fps(
                ...
                data,
                {
                    "env_id": args.env_id,
                    "obs_mode": obs_mode,
                    "camera_width": cam_size,
                    "camera_height": cam_size,
                    "num_cameras": 1,
                },
                annotate_label="env.step/gpu_mem_use",
            )
    ...
    for obs_mode in ["rgb", "rgb+depth"]:
        ...
        for i, (exp_name, df) in enumerate(data.items()):
            ...
            for j, (x, y) in enumerate(zip(df["camera_width"], df["env.step/fps"])):
                ax.annotate(
                    f"{df['num_envs'].iloc[j]} envs", (x, y), textcoords="offset points", xytext=(0, 5), ha="center"
                )
            ax.plot(
                df["camera_width"],
                df["env.step/fps"],
    ...
    for camera_size in [80, 128, 160, 224, 256, 512]:
        for obs_mode in ["rgb", "rgb+depth"]:
            ...
            ax.set_ylabel("FPS")
            ax.set_title(
                f"{args.env_id}: Highest RGB FPS vs Number of {camera_size}x{camera_size} Cameras under 16GB GPU memory"
            )
            for i, (exp_name, df) in enumerate(data.items()):
                df = df[df["env_id"] == args.env_id]
                df = df[df["env.step/gpu_mem_use"] < 16 * 1024 * 1024 * 1024]
    ...
    draw_bar_plot_envs_vs_fps(
        ax, data, {"env_id": args.env_id, "obs_mode": "state"}, annotate_label="env.step/gpu_mem_use"
    )
    save_path = osp.join(root_save_path, "fps_num_envs_state.png")
    fig.savefig(save_path)
    plt.close(fig)
    print(f"Saved figure to {save_path}")
    ...
    for col in fixed_trajectory_cols:
        ...
        draw_bar_plot_envs_vs_fps(
            ax, data, {"env_id": args.env_id, "obs_mode": "state"}, yname=col, annotate_label="env.step/gpu_mem_use"
        )
    ...
    if "ManiSkill3" in data.keys():
        ...
        for i, env_id in enumerate(env_ids):
            ...
            for x, y, mem_use in zip(env_df["num_envs"], env_df["env.step/fps"], env_df["env.step/gpu_mem_use"]):
                ax.annotate(
                    f"{mem_use / (1024 * 1024 * 1024):0.1f} GB",
                    (x, y),
                    textcoords="offset points",
                    xytext=(0, 5),
                    ha="center",
