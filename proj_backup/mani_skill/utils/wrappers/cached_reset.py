...
class CachedResetWrapper(gym.Wrapper):
    ...
    def __init__(
        self,
        env: gym.Env,
        reset_to_env_states: dict | None = None,
        config: CachedResetsConfig | dict = CachedResetsConfig(),
    ):
        super().__init__(env)
        ...
        if reset_to_env_states is not None:
            ...
            while self._num_cached_resets < self.cached_resets_config.num_resets:
                ...
                self._cached_resets_obs_buffer.append(common.to_tensor(obs, device=self.cached_resets_config.device))
                self._cached_resets_env_states.append(common.to_tensor(state, device=self.cached_resets_config.device))
    ...
    def reset(self, *args, seed: int | list[int] | None = None, options: dict | None = None, **kwargs):
        env_idx = None
        if options is None:
            options = dict()
