...
class LeRobotRealAgent(BaseRealAgent):
    ...
    def reset(self, qpos: Array):
        ...
        for _ in range(int(20 * freq)):
            ...
            delta_step = (qpos - target_pos).clip(min=-max_rad_per_step, max=max_rad_per_step)
    ...
    def capture_sensor_data(self, sensor_names: list[str] | None = None):
        sensor_obs = dict()
    ...
    def get_sensor_data(self, sensor_names: list[str] | None = None):
        if self._captured_sensor_data is None:
            raise RuntimeError("No sensor data captured yet. Please call capture_sensor_data() first.")
        if sensor_names is None:
            return self._captured_sensor_data
        else:
            return {k: v for k, v in self._captured_sensor_data.items() if k in sensor_names}
