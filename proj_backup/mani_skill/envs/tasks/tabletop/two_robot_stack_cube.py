...
class TwoRobotStackCube(BaseEnv):
    ...
    **Success Conditions:**
    ...
    _sample_video_link = (
        "https://github.com/haosulab/ManiSkill/raw/main/figures/environment_demos/TwoRobotStackCube-v1_rt.mp4"
    )
    ...
    def __init__(self, *args, robot_uids=("panda_wristcam", "panda_wristcam"), robot_init_qpos_noise=0.02, **kwargs):
    ...
    def evaluate(self):
        ...
        cubeB_to_goal_dist = torch.linalg.norm(self.cubeB.pose.p[:, :2] - self.goal_region.pose.p[..., :2], axis=1)
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        ...
        cubeA_to_left_arm_tcp_dist = torch.linalg.norm(self.left_agent.tcp.pose.p - self.cubeA.pose.p, axis=1)
        ...
        right_arm_to_push_pose_dist = torch.linalg.norm(right_arm_push_pose.p - self.right_agent.tcp.pose.p, axis=1)
        reach_reward = (
            1 - torch.tanh(5 * cubeA_to_left_arm_tcp_dist) + 1 - torch.tanh(5 * right_arm_to_push_pose_dist)
        ) / 2
        ...
        cubeB_to_goal_dist = torch.linalg.norm(cubeB_pos[:, :2] - self.goal_region.pose.p[..., :2], axis=1)
        ...
        goal_xyz = torch.hstack([cubeB_pos[:, :2], (cubeB_pos[:, 2] + self.cube_half_size[2] * 2)[:, None]])
        ...
        right_arm_leave_reward = 1 - torch.tanh(5 * (self.right_agent.tcp.pose.p[:, 1] - 0.2).abs())
        ...
        ungrasp_reward_left = torch.sum(self.left_agent.robot.get_qpos()[:, -2:], axis=1) / gripper_width
        ungrasp_reward_left[~info["is_cubeA_grasped"]] = 1.0
        ungrasp_reward_right = torch.sum(self.right_agent.robot.get_qpos()[:, -2:], axis=1) / gripper_width
        ungrasp_reward_right[~info["is_cubeB_grasped"]] = 1.0

        reward[cubes_placed] = 8 + (ungrasp_reward_left + ungrasp_reward_right)[cubes_placed] / 2
