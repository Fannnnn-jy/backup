...
def load_ai2thor_metadata():
    with open(
        str(Path(ASSET_DIR) / "scene_datasets/ai2thor/ai2thor-hab/configs/object_semantic_id_mapping.json"),
    ) as f:
...
SCENE_SOURCE_TO_DATASET: dict[str, SceneDataset] = {
    "ProcTHOR": SceneDataset(
        ...
        dataset_path=osp.join(ASSET_DIR, "scene_datasets/ai2thor/ai2thor-hab/configs/scenes/ProcTHOR"),
    ),
    "ArchitecTHOR": SceneDataset(
        metadata_path="ArchitecTHOR.json",
        dataset_path=osp.join(ASSET_DIR, "scene_datasets/ai2thor/ai2thor-hab/configs/scenes/ArchitecTHOR"),
    ),
    "iTHOR": SceneDataset(
        metadata_path="iTHOR.json",
        dataset_path=osp.join(ASSET_DIR, "scene_datasets/ai2thor/ai2thor-hab/configs/scenes/iTHOR"),
    ),
    "RoboTHOR": SceneDataset(
        metadata_path="RoboTHOR.json",
        dataset_path=osp.join(ASSET_DIR, "scene_datasets/ai2thor/ai2thor-hab/configs/scenes/RoboTHOR"),
    ),
}
