...
def main(cfg):
    ...
    try:
        ...
        mission_error = exc
        print(f"[active-gs] mission ended with error: {exc}")
    finally:
        # save keyframe poses to baseline directory, including partial poses on failure
        if not cfg.debug and mission_recorder is not None:
            ...
            metadata = {
                ...
                "status": "incomplete" if mission_error is not None else "completed",
                "completed_views": int(len(mission_recorder.selected_pose_list)),
            }
            if mission_error is not None:
                metadata["failure_reason"] = str(mission_error)
