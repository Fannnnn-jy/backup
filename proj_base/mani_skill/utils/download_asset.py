...
def download(
    ...
    try:
        ...
        tmp_filename, _ = urllib.request.urlretrieve(data_source.url, reporthook=show_progress)
        if verbose:
            pbar.close()
        ...
        raise err
    # Verify checksum
    if data_source.checksum is not None and data_source.checksum != sha256sum(tmp_filename):
        raise OSError(f"Downloaded file's SHA-256 hash does not match record: {data_source.url}")
        ...
        with zipfile.ZipFile(tmp_filename, "r") as zip_ref:
            ...
            for file in zip_ref.filelist:
                ...
                if shared_base_dir is None:
                    ...
                    shared_base_dir = None
            if shared_base_dir is not None and shared_base_dir != osp.basename(data_source.target_path):
                os.rename(
...
def parse_args(args=None):
    ...
    parser.add_argument("-y", "--non-interactive", action="store_true", help="Disable prompts.")
    parser.add_argument("-o", "--output-dir", type=str, help="Directory to save assets.")
...
def main(args):
    ...
    if args.uid == "all":
        ...
        raise KeyError(f"{args.uid} not found.")

    for i, uid in enumerate(uids):
        if show_progress and verbose:
            print(f"Downloading assets for {args.uid}: {i + 1}/{len(uids)}")
