...
def _import_sapien():
    try:
        ...
        raise ImportError(
            "sapien is required for ManiSkillActiveMappingEnv. Install maniskill3 and its "
            "sapien dependency."
        ) from exc
    return sapien
...
class ManiSkillActiveMappingEnv(ActiveMappingEnv):
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self._step_count = 0
        if (
            self._gleam_dataset is not None
            and self.cfg.gleam_scene_index < 0
            and self.cfg.gleam_resample_scene
        ):
    ...
    def step(self, action: np.ndarray):
        ...
        if crash:
            ...
            if self._reward_valid_mask is None:
                ...
                all_valid_visited = np.logical_or(
                    self._reward_visited, ~self._reward_valid_mask
                ).all()
    ...
    def _update_camera_poses(self) -> None:
        ...
        cam_pos = np.array(
            [self._position[0], self._position[1], z_pos], dtype=np.float32
        )
    ...
    def _get_depth_images(self) -> List[np.ndarray]:
        ...
        if not depth_images:
            depth_images = [
                self.rng.random(self.cfg.depth_image_shape, dtype=np.float32)
                for _ in range(self.cfg.max_depth_frames)
            ]
        return depth_images
            ...
            if hasattr(cam, "get_picture_names"):
                ...
                if candidate_names:
                    filtered = [
                        ...
                        if "depth" in name.lower() or "position" in name.lower()
                    ]
                    if filtered:
                        names = filtered
            if buffer_name:
