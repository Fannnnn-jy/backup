...
class WidowX250SBridgeDatasetFlatTable(WidowX250S):
    ...
    def _controller_configs(self):
        ...
        arm_pd_ee_target_delta_pose_align2 = PDEEPoseControllerConfig(**arm_common_kwargs, use_target=True)
        ...
        controller = dict(arm=arm_pd_ee_target_delta_pose_align2, gripper=gripper_pd_joint_pos)
    ...
    def _sensor_configs(self):
        return [
            CameraConfig(
                ...
                pose=sapien.Pose([-0.00300001, -0.21, 0.39], [-0.907313, 0.0782, -0.36434, -0.194741]),
                entity_uid="base_link",
                width=640,
                # fov=1.5,
                height=480,
...
class BaseBridgeEnv(BaseDigitalTwinEnv):
        ...
        **kwargs,
        ...
        if self.scene_setting == "flat_table":
            self.rgb_overlay_paths = {
                "3rd_view_camera": str(BRIDGE_DATASET_ASSET_PATH / "real_inpainting/bridge_real_eval_1.png")
            }
            robot_cls = WidowX250SBridgeDatasetFlatTable
        elif self.scene_setting == "sink":
            self.rgb_overlay_paths = {
                "3rd_view_camera": str(BRIDGE_DATASET_ASSET_PATH / "real_inpainting/bridge_sink.png")
            }
            robot_cls = WidowX250SBridgeDatasetSink

        self.model_db: dict[str, dict] = io_utils.load_json(BRIDGE_DATASET_ASSET_PATH / "custom/" / self.MODEL_JSON)
    ...
    def _load_scene(self, options: dict):
        ...
        self.xyz_configs = common.to_tensor(self.xyz_configs, device=self.device).to(torch.float32)
        self.quat_configs = common.to_tensor(self.quat_configs, device=self.device).to(torch.float32)
        ...
        if model_scales is None:
            ...
            for model_id in [self.source_obj_name, self.target_obj_name]:
                ...
                if this_available_model_scales is None:
                    ...
                    model_scales[model_id] = self.np_random.choice(this_available_model_scales)
        ...
        for model_id in [self.source_obj_name, self.target_obj_name]:
            ...
            if "bbox" in model_info:
                ...
                model_bbox_sizes[model_id] = common.to_tensor(bbox_size * model_scale, device=self.device)
            else:
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            if "episode_id" in options:
                ...
                pos_episode_ids = (options["episode_id"] % (len(self.xyz_configs) * len(self.quat_configs))) // len(
                    self.quat_configs
                )
            ...
            for i, actor in enumerate(self.objs.values()):
                ...
                actor.set_pose(Pose.create_from_pq(p=xyz, q=self.quat_configs[quat_episode_ids, i]))
            ...
            if self.scene_setting == "flat_table":
                ...
                self.agent.robot.set_pose(sapien.Pose([0.147, 0.028, 0.870], q=[0, 0, 0, 1]))
            elif self.scene_setting == "sink":
                qpos = np.array([
                    -0.2600599,
                    -0.12875618,
                ...
                self.agent.robot.set_pose(sapien.Pose([0.127, 0.060, 0.85], q=[0, 0, 0, 1]))
            ...
            self.episode_obj_xyzs_after_settle = {obj_name: self.objs[obj_name].pose.p for obj_name in self.objs.keys()}
        ...
        **kwargs,
        ...
        for obj_name in self.objs.keys():
            ...
            if obj.name == self.source_obj_name:
                continue
            other_obj_xy_move_dist.append(torch.linalg.norm(obj_xyz_after_settle[:, :2] - obj.pose.p[:, :2], dim=1))
        ...
        xy_flag = torch.linalg.norm(offset[:, :2], dim=1) <= torch.linalg.norm(tgt_obj_half_length_bbox[:2]) + 0.003
        z_flag = (offset[:, 2] > 0) & (
        ...
        if success_require_src_completely_on_target:
            ...
            contact_forces = self.scene.get_pairwise_contact_forces(source_object, target_object)
