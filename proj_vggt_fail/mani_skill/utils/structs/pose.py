...
def to_batched_tensor(x: list | Array, device: Device | None = None):
    if x is None:
        return None
...
def vectorize_pose(pose: sapien.Pose | Pose | Array, device: Device | None = None) -> torch.Tensor:
    """
...
def to_sapien_pose(pose: torch.Tensor | sapien.Pose | Pose) -> sapien.Pose:
    ...
    if isinstance(pose, sapien.Pose):
        ...
        pose = pose.raw_pose
        assert len(pose.shape) == 1 or (len(pose.shape) == 2 and pose.shape[0] == 1), (
