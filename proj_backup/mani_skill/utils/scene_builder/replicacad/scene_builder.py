...
class ReplicaCADSceneBuilder(SceneBuilder):
    robot_initial_pose = sapien.Pose(p=[-1, 0, 0.02])  # generally a safe initial spawn pose for the Fetch robot
    ...
    def build(self, build_config_idxs: int | list[int]):
        ...
        for bci in np.unique(build_config_idxs):
            ...
            for obj_num, obj_meta in enumerate(build_config_json["object_instances"]):
                ...
                if "collision_asset" in obj_config:
                    collision_file = osp.join(osp.dirname(obj_config_path), obj_config["collision_asset"])
                builder = self.scene.create_actor_builder()
                ...
                if obj_meta["motion_type"] == "DYNAMIC":
                    ...
                    if "use_bounding_box_for_collision" in obj_config and obj_config["use_bounding_box_for_collision"]:
            ...
            for i, articulated_meta in enumerate(build_config_json["articulated_object_instances"]):
                ...
                for env_num in env_idx:
                    self.articulations[f"env-{env_num}_{articulation_name}"] = articulation
                    self.scene_objects[f"env-{env_num}_{articulation_name}"] = articulation
            ...
            if self._navigable_positions[bci] is None:
                mesh_fp = (
                    Path(ASSET_DIR)
                    / "scene_datasets/replica_cad_dataset/configs/scenes"
                    / (Path(self.build_configs[bci]).stem + f".{str(self.env.robot_uids)}.navigable_positions.obj")
                )
                if mesh_fp.exists():
        ...
        if self.ray_traced_lighting:
            for sub_scene in self.scene.sub_scenes:
                sub_scene.set_environment_map(str((Path(__file__).parent / "autumn_field_puresky_4k.hdr").absolute()))
        else:
    ...
    def ray_traced_lighting(self) -> bool:
        return self.env._custom_human_render_camera_configs.get("shader_pack", None) in [
            "rt",
            "rt-fast",
        ]
