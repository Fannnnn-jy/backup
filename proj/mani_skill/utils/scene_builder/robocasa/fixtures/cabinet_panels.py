...
class CabinetPanel(MujocoObject):
    ...
    def set_geom_dimensions(self, sizes, positions, rotated=False):
        ...
        for i, ((part_name, size), (_, position)) in enumerate(zip(sizes.items(), positions.items())):
            col_record = self.actor_builder.collision_records[self.geom_names.index(part_name)]
    ...
    def _set_texture(self):
        ...
        for visual_record in self.actor_builder.visual_records:
            ...
            if shortname in self.geom_names_set and shortname not in self.material_overrides:
    ...
    def _add_handle(self):
        ...
        if self.handle_hpos == "left":
            ...
            raise NotImplementedError
        for col_record, vis_record in zip(handle.actor_builder.collision_records, handle.actor_builder.visual_records):
...
class RaisedCabinetPanel(CabinetPanel):
    ...
    def __init__(self, name, trim_th=0.02, trim_size=0.08, raised_gap=0.01, *args, **kwargs):
        self.trim_th = trim_th
        self.trim_size = trim_size
...
class FullWindowedCabinetPanel(CabinetPanel):
    ...
    def __init__(self, name, trim_th=0.02, trim_size=0.08, opacity=0.5, *args, **kwargs):
        self.trim_th = trim_th
        self.trim_size = trim_size
