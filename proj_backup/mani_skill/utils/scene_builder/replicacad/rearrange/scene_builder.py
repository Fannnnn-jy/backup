...
class ReplicaCADRearrangeSceneBuilder(ReplicaCADSceneBuilder):
    ...
    def __init__(self, env):
        ...
        for rc in self._rearrange_configs:
            ...
            self.used_build_config_idxs.add(bc_to_idx[Path(episode_json["scene_id"]).name])

    def build(self, build_config_idxs: list[int], init_config_names: list | None = None):
        ...
        for rc in init_config_names:
            ...
            for actor_id, transformation in episode_json["rigid_objs"]:
                ...
                objects[actor_id].append(obj_transform * sapien.Pose(transformation) * world_transform)
        ...
        self.init_configs: list[list[dict[str, list[sapien.Pose]]]] = [
            [default_object_poses[rc] for rc in self.rcad_to_rearrange_configs[self.build_configs[bci]]]
        ...
        self.num_init_configs_per_build_config = [
            len(self.rcad_to_rearrange_configs[self.build_configs[bci]]) for bci in build_config_idxs
        ]
        ...
        _running_default_hidden_pos_by_env = dict(
            (env_num, copy.deepcopy(DEFAULT_HIDDEN_POS)) for env_num in range(len(build_config_idxs))
        )
        ...
        for env_num, bci in enumerate(build_config_idxs):
            ...
            for actor_id, num_objs in num_ycb_objs_to_build.items():
                for no in range(num_objs):
                    ...
                    builder.initial_pose = sapien.Pose(p=_running_default_hidden_pos_by_env[env_num])
                    actor = builder.build(name=obj_instance_name)

                    ycb_objs[actor_id].append(actor)
                    self._default_hidden_poses[actor] = sapien.Pose(p=_running_default_hidden_pos_by_env[env_num])
        ...
        self.rcad_config_to_rearrange_ao_states: dict[str, list[list[tuple[Articulation, torch.Tensor]]]] = defaultdict(
            list
        )
        for env_num, bci in enumerate(build_config_idxs):
            rcad_config = self.build_configs[bci]
            ...
            for rearrange_config in self.rcad_to_rearrange_configs[rcad_config]:
                ...
                for base_articulation_id, qpos_dict in episode_ao_states.items():
                    ...
                    articulation = self.articulations[f"env-{env_num}_{aid[:-1]}-{int(anum)}"]
                    base_qpos = torch.zeros(articulation.max_dof, device=self.env.device)
                    for link_num, qpos_val in qpos_dict.items():
                        qpos_idx = articulation.active_joints.index(articulation.joints[int(link_num)])
    ...
    def initialize(self, env_idx: torch.Tensor, init_config_idxs: list[int]):
        ...
        init_config_idxs: torch.Tensor = common.to_tensor(init_config_idxs, device=self.env.device).to(torch.int)
        if env_idx.numel() != init_config_idxs.numel():
            init_config_idxs = init_config_idxs[env_idx]

        # get sampled init configs
        sampled_init_configs = [self.init_configs[env_num][idx] for env_num, idx in zip(env_idx, init_config_idxs)]
        ...
        for env_num, init_poses in zip(env_idx, sampled_init_configs):
            ...
            for obj_name in ycb_objs:
                for obj, pose in itertools.zip_longest(ycb_objs[obj_name], init_poses[obj_name], fillvalue=None):
                    if pose is None:
                        self.hide_actor(obj)
                    else:
                        temp_p = pose.p
        ...
        for env_num, ici in zip(env_idx, init_config_idxs):
            ...
            for articulation, qpos in ao_states:
                ...
                reset_idxs = [bn for bn, en in enumerate(articulation._scene_idxs) if en in env_idx]
    ...
    def sample_build_config_idxs(self):
        ...
        return [
            ...
            for i in torch.randint(low=0, high=len(used_build_config_idxs), size=(self.env.num_envs,)).tolist()
        ]
    ...
    def sample_init_config_idxs(self):
        ...
        return (torch.randint(2**63 - 1, size=size) % (high - low).int() + low).int().tolist()
