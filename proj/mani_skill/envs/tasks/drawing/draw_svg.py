    ...
    def _load_scene(self, options: dict):
        ...
        def bezier_points(points, num_points=5):
            ...
            if len(points) == 3:
                ...
                p0, p1, p2, p3 = points
                points = (1 - t) ** 3 * p0 + 3 * t * (1 - t) ** 2 * p1 + 3 * t**2 * (1 - t) * p2 + t**3 * p3
        ...
        self.ref_dist = torch.zeros((self.num_envs, self.original_points.shape[0]), device=self.device).to(bool)
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            if hasattr(self, "vertices"):
                self.points[env_idx] = torch.from_numpy(np.tile(self.original_points, (b, 1, 1))).to(
                    self.device
                )  # b, 3, 3
            else:
                self.points = torch.from_numpy(np.tile(self.original_points, (b, 1, 1))).to(self.device)

            self.points[env_idx] = (rot_mat.double() @ self.points[env_idx].transpose(-1, -2).double()).transpose(
                -1, -2
            )  # rotation matrix
            self.points[env_idx] += target_pos.unsqueeze(1)
            self.dots_dist[env_idx] = torch.ones((self.num_envs, 500)) * -1
            self.ref_dist[env_idx] = torch.zeros((self.num_envs, self.original_points.shape[0])).to(bool)
    ...
    def success_check(self):
        if self.draw_step > 0:
            ...
            dist = (
                torch.sqrt(
                    torch.sum(
                        (current_dot[:, :, None, :2] - self.points[:, None, :, :2]) ** 2,
                        dim=-1,
                    )
                )
                < self.THRESHOLD
            )
            self.ref_dist = torch.logical_or(self.ref_dist, (1 - z_mask.int()) * dist.reshape((self.num_envs, -1)))
