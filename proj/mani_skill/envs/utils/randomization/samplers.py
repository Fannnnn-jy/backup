...
class UniformPlacementSampler:
    ...
    def sample(self, radius, max_trials, append=True, verbose=False):
        ...
        if self.fixture_positions is None:
            sampled_pos = torch.rand((self.batch_size, self._bounds.shape[1])) * self._ranges + self._bounds[0]
        else:
            pass_mask = torch.zeros((self.batch_size), dtype=bool)
            sampled_pos = torch.zeros((self.batch_size, self._bounds.shape[1]))
            for i in range(max_trials):
                pos = torch.rand((self.batch_size, self._bounds.shape[1])) * self._ranges + self._bounds[0]  # (B, d)
                dist = torch.linalg.norm(pos - self.fixture_positions, axis=-1)  # (n, B)
                ...
                if torch.all(pass_mask):
                    if verbose:
                        print(f"Found valid set of {self.batch_size=} samples at {i}-th trial")
                    break
            else:
                if verbose:
        ...
        if append:
            if self.fixture_positions is None:
                ...
                self.fixture_positions = torch.concat([self.fixture_positions, sampled_pos[None, ...]])
