...
class HumanoidEnvBase(BaseEnv):
    ...
    def extremities(self, info):
        ...
        for side in ("left_", "right_"):
            for limb in ("hand", "foot"):
                torso_to_limb = (self.agent.robot.links_map[side + limb].pose.p - torso_pos).view(-1, 1, 3)
                positions.append(
    ...
    def evaluate(self) -> dict:
        return dict(
            torso_xmat=self.agent.robot.links_map["torso"].pose.to_transformation_matrix(),
    ...
    def _load_scene(self, options: dict):
        ...
        self.active_links = [link for link in self.agent.robot.get_links() if "dummy" not in link.name]
    ...
    def control_rew(self, action: Array):
        return (
            rewards.tolerance(action, margin=1, value_at_margin=0, sigmoid="quadratic").mean(dim=-1).view(-1)
        )  # (b, a) -> (b)
...
class HumanoidEnvStandard(HumanoidEnvBase):
    ...
    def _get_obs_state_dict(self, info: dict):
        ...
        return dict(
            ...
            link_linvels=torch.stack([link.get_linear_velocity() for link in self.active_links], dim=1).view(
                -1, 16 * 3
            ),
            link_angvels=torch.stack([link.get_angular_velocity() for link in self.active_links], dim=1).view(
                -1, 16 * 3
            ),
...
class HumanoidStand(HumanoidEnvStandard):
    ...
    **Fail Conditions:**
    ...
    def _get_obs_state_dict(self, info: dict):
        ...
        root_pose_mat = self.agent.robot.links_map["dummy_root_0"].pose.to_transformation_matrix()[:, :3, :3]
        lin_vels = [link.get_linear_velocity() for link in self.active_links]  # (links, b, 3)
        ang_vels = [link.get_angular_velocity() for link in self.active_links]  # (links, b, 3)
        non_ego_vels = torch.stack(
            [*lin_vels, *ang_vels, info["cmass_linvel"]], dim=1
        )  # (b, len(lin_vels)+len(ang_vels)+1, 3)
        ego_vels = (non_ego_vels @ root_pose_mat).view(-1, (len(lin_vels) + len(ang_vels) + 1) * 3)
        return dict(
            agent=self._get_obs_agent(),  # (b, 21*2) root joint not included in our qpos
            head_height=self.head_height,  # (b,1)
    ...
    def compute_normalized_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        ...
        stand_rew = small_control * self.standing_rew() * self.upright_rew(info) * self.dont_move_rew(info)
...
class HumanoidWalk(HumanoidEnvStandard):
    ...
    **Fail Conditions:**
    ...
    def compute_normalized_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        small_control = (4 + self.control_rew(action)) / 5
        walk_rew = small_control * self.move_x_rew(info, _WALK_SPEED) * self.upright_rew(info) * self.standing_rew()
        alive_rew = 1
...
class HumanoidRun(HumanoidEnvStandard):
    ...
    **Fail Conditions:**
    ...
    def compute_normalized_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        ...
        alive_rew = 5
        return rew_scale * (alive_rew + 1.25 * run_x_rew - 0.1 * action.pow(2).sum(dim=-1))
