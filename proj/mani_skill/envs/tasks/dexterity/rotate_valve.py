...
class RotateValveEnv(BaseEnv):
    ...
    def _load_articulations(self):
        ...
        if self.difficulty_level == 0:
            ...
            valve_angles_list = base_angles * int(self.num_envs // 3) + base_angles[: int(self.num_envs % 3)]
        elif self.difficulty_level == 2:
            num_valve_head = self._batched_episode_rng.randint(3, 6)
            valve_angles_list = [
                sample_valve_angles(num_head, self._batched_episode_rng[i]) for i, num_head in enumerate(num_valve_head)
            ]
    ...
    def _initialize_agent(self, env_idx: torch.Tensor):
        with torch.device(self.device):
            ...
            self.agent.robot.set_pose(Pose.create_from_pq(torch.tensor([0.0, 0, 0.28]), torch.tensor([0, 0, -1, 0])))
...
def sample_valve_angles(
    ...
    logger.warn(f"sample_valve_angles reach max attempts {num_max_attempts}. Will use the default valve angles.")
