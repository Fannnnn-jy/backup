...
class ArticulationBuilder(SapienArticulationBuilder):
    ...
    def _build_entities(self, fix_root_link=None, name_prefix="", initial_pose=sapien.Pose()):
        entities = []
        links = []
        for b in self.link_builders:
            ...
            link_component = b.build_physx_component(links[b.parent.index] if b.parent else None)
    ...
    def build(self, name=None, fix_root_link=None, build_mimic_joints=True) -> Articulation:
        assert self.scene is not None
        if name is not None:
            self.set_name(name)
        assert self.name is not None and self.name != "" and self.name not in self.scene.articulations, (
                ...
                for mimic in self.mimic_joint_records:
                    ...
                    if joint.parent_link == mimic_joint.child_link:
                        if joint.parent_link.parent is None:
                            logger.warn(f"Skipping adding fixed tendon for {joint.name}")
