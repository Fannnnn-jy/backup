...
class ArticulationJoint(BaseStruct[physx.PhysxArticulationJoint]):
    ...
    def _data_index(self):
        return torch.tensor(
            [px_articulation.gpu_index for px_articulation in self._physx_articulations],
            device=self.device,
            dtype=torch.int32,
        )
    ...
    def qpos(self):
        ...
        """
        assert self.active_index is not None, "Inactive joints do not have qpos/qvel values"
        if self.scene.gpu_sim_enabled:
            return self.px.cuda_articulation_qpos.torch()[self._data_index, self.active_index]
        else:
    ...
    def qvel(self):
        ...
        assert self.active_index is not None, "Inactive joints do not have qpos/qvel values"
        if self.scene.gpu_sim_enabled:
            return self.px.cuda_articulation_qvel.torch()[self._data_index, self.active_index]
        else:
    ...
    def drive_target(self) -> torch.Tensor:
        assert self.active_index is not None, "Cannot get drive position targets of inactive joints"
        if self.scene.gpu_sim_enabled:
    ...
    def drive_target(self, arg1: Array) -> None:
        arg1 = common.to_tensor(arg1, device=self.device)
        assert self.active_index is not None, "Cannot set drive position targets of inactive joints"
        if self.scene.gpu_sim_enabled:
    ...
    def drive_velocity_target(self) -> torch.Tensor:
        assert self.active_index is not None, "Cannot get drive velocity targets of inactive joints"
        if self.scene.gpu_sim_enabled:
    ...
    def drive_velocity_target(self, arg1: Array) -> None:
        arg1 = common.to_tensor(arg1, device=self.device)
        assert self.active_index is not None, "Cannot set drive velocity targets of inactive joints"
        if self.scene.gpu_sim_enabled:
    ...
    def limits(self) -> torch.Tensor:
        ...
        return common.to_tensor(np.array([obj.limits[0] for obj in self._objs]), device=self.device)
    ...
    def pose_in_child(self):
        raw_poses = np.stack([np.concatenate([x.pose_in_child.p, x.pose_in_child.q]) for x in self._objs])
    ...
    def pose_in_parent(self):
        raw_poses = np.stack([np.concatenate([x.pose_in_parent.p, x.pose_in_parent.q]) for x in self._objs])
        return Pose.create(common.to_tensor(raw_poses, device=self.device))
    ...
    def type(
        self,
    ) -> list[typing.Literal["fixed", "revolute", "revolute_unwrapped", "prismatic", "free"]]:
    ...
    def type(
        self,
        arg1: typing.Literal["fixed", "revolute", "revolute_unwrapped", "prismatic", "free"],
    ) -> None:
        if self.scene.gpu_sim_enabled:
