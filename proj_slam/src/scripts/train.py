...
# Ensure this copy's ``src`` package is used instead of the editable-installed
# ``proj/src``.  We must remove any cached ``src`` module **and** insert our
# project root before the editable-install path so that subsequent ``from
# src.xxx`` imports resolve here.
...
def _build_env_cfg(cfg: TrainConfig) -> ActiveMappingConfig:
    ...
    policies_need_xyz_delta_quat = {"ActorCriticRGBCNNRecurrent", "SLAMFormerRelativeActorCritic"}
...
class InstrumentedOnPolicyRunner:
    def __init__(
        ...
        *,
        ...
        # Try to share SLAMFormer model between policy and reconstruction evaluator
        shared_slam_model = None
        policy = self._runner.alg.policy
        feat_ext = getattr(policy, "feature_extractor", None)
        if feat_ext is not None:
            shared_slam_model = getattr(feat_ext, "_slamformer_model", None)
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False) -> None:
        ...
        if init_at_random_ep_len:
            max_ep = int(runner.env.max_episode_length)
            warmup = getattr(runner.alg.policy, "warmup_steps", 0)
            min_remaining = warmup + 1  # at least 1 post-warmup step
            high = max(max_ep - min_remaining, 1)
            runner.env.episode_length_buf = torch.randint_like(
                runner.env.episode_length_buf, high=high
            )
