...
try:
    ...
    raise ImportError("pytorch_kinematics_ms not installed. Install with pip install pytorch_kinematics_ms")
import torch
...
class Kinematics:
    def __init__(
        ...
        self._kinematic_chain_joint_names = set(node.get("name") for node in xml if node.tag == "joint")
        self._kinematic_chain_link_names = set(node.get("name") for node in xml if node.tag == "link")
        self.active_ancestor_joints = [x for x in active_ancestor_joints if x.name in self._kinematic_chain_joint_names]
    ...
    def _setup_cpu(self):
        ...
        joint_order = [j.name for j in self.articulation.active_joints if j.name in self._kinematic_chain_joint_names]
        link_order = [l.name for l in self.articulation.links if l.name in self._kinematic_chain_link_names]

        self.pmodel = PinocchioModel(xml, [0, 0, -9.81])
        self.pmodel.set_joint_order(joint_order)
        self.pmodel.set_link_order(link_order)

        controlled_joint_names = [self.articulation.active_joints[i].name for i in self.active_joint_indices]
    ...
    def _setup_gpu(self):
        ...
        self.active_ancestor_joint_idxs = [(x.active_index[0]).cpu().item() for x in self.active_ancestor_joints]
        self.controlled_joints_idx_in_qmask = [
            self.active_ancestor_joint_idxs.index(idx) for idx in self.active_joint_indices
        ]

        self.qmask = torch.zeros(len(self.active_ancestor_joints), dtype=bool, device=self.device)
    ...
    def compute_ik(
        ...
        if self.use_gpu_ik:
            ...
            if not is_delta_pose:
                if current_pose is None:
                    ...
                    current_pose = Pose.create_from_pq(
                        current_pose[:, :3, 3],
                        rotation_conversions.matrix_to_quaternion(current_pose[:, :3, :3]),
                    )
            ...
            if solver_config["type"] == "levenberg_marquardt":
                ...
                lfs_A = torch.bmm(J_T, jacobian) + lambd * torch.eye(len(self.qmask), device=self.device)
            ...
            return q0[:, self.qmask] + solver_config["alpha"] * delta_joint_pos.squeeze(-1)
        else:
            ...
            if success:
                return common.to_tensor([result[self.pmodel_controlled_joint_indices]], device=self.device)
            else:
                return None
