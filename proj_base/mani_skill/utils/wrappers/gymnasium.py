...
class CPUGymWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        super().__init__(env)
        assert self.base_env.num_envs == 1, "This wrapper is only for environments without parallelization"
        assert not self.base_env.gpu_sim_enabled, "This wrapper is only for environments on the CPU backend"
