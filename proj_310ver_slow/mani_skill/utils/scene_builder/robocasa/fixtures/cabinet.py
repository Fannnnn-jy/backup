...
class Cabinet(Fixture):
    ...
    def _add_door(
        ...
        if parent_actor_builder is None and parent_link_builder is None:
            raise ValueError("Either parent_actor_builder or parent_link_builder must be provided")
        ...
        if parent_link_builder is not None:
            ...
            for door_collision_record, door_visual_record in zip(
                ...
                if link_builder.joint_record.joint_type == "prismatic":
                    door_collision_record.pose = sapien.Pose(p=np.array(pos)) * door_collision_record.pose
                    door_visual_record.pose = sapien.Pose(p=np.array(pos)) * door_visual_record.pose
...
class SingleCabinet(Cabinet):
    ...
    def _create_cab(self):
        ...
        for i, ((part_name, size), (_, position)) in enumerate(zip(sizes.items(), positions.items())):
        ...
        if self.orientation == "left":
            joint_record.pose_in_parent = sapien.Pose(p=[-x + th, -y, 0], q=joint_record.pose_in_parent.q)
            joint_record.pose_in_child = sapien.Pose([-x + th, -y, 0], q=joint_record.pose_in_child.q)
...
class HingeCabinet(Cabinet):
        ...
        for side in ["left", "right"]:
            self._add_door(
                ...
                parent_link_builder=next(
                    lb for lb in self.articulation_builder.link_builders if f"hinge{side}door" in lb.name
                ),
    ...
    def set_door_state(self, min, max, env, rng):
        ...
        env.sim.data.set_joint_qpos(f"{self.name}_rightdoorhinge", rng.uniform(desired_min, desired_max))

        env.sim.data.set_joint_qpos(f"{self.name}_leftdoorhinge", -rng.uniform(desired_min, desired_max))
...
class OpenCabinet(Cabinet):
    ...
    def _create_cab(self):
        ...
        shelf_z_positions = np.linspace(start=th / 2, stop=z - th / 2, num=self.num_shelves, endpoint=False) - z / 2
        ...
        for i in range(self.num_shelves):
            ...
            # TODO
            self.actor_builder.collision_records.append(shelf.actor_builder.collision_records[0])
            self.actor_builder.visual_records.append(shelf.actor_builder.visual_records[0])
...
class Drawer(Cabinet):
    ...
    def _create_cab(self):
        ...
        joint_record = self.articulation_builder.link_builders[2].joint_record
        joint_record.pose_in_parent = sapien.Pose(p=[0, y, 0], q=joint_record.pose_in_parent.q)
